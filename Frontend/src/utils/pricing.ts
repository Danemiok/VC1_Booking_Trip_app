export type TripPricingInput = {
  tripData?: any;
  selectedActivities?: Array<{ price?: number; guests?: number }>;
};
...
export const calculateTripPricing = ({ tripData, selectedActivities = [] }: TripPricingInput) => {
  ...
  const hotelDiscount =
    hotelHasPromotion && hotelNightlyBase > 0 && hotelNightlyDiscounted > 0
      ? Math.max(0, (hotelNightlyBase - hotelNightlyDiscounted) * hotelNights)
      : 0;
  const hotelOriginalTotal = hotelTotal + hotelDiscount;

  const rentalDailyBase = toNumber(
    rental?.originalDailyPrice ?? rental?.originalPrice ?? rental?.original_price ?? rental?.dailyPrice,
    0,
  );
  const rentalDailyDiscounted = toNumber(
    rental?.discountedDailyPrice ?? rental?.discountedPrice ?? rental?.discounted_price ?? rental?.dailyPrice,
    0,
  );
  ...
  const rentalDiscount =
    rentalHasPromotion && rentalDailyBase > 0 && rentalDailyDiscounted > 0
