...
export const Register = ({ onSwitchToLogin, onBack, onSuccess, onClose, }) => {
    ...
    const getPasswordRequirementIcon = (isValid) => (isValid ? (<Check className="w-3 h-3 text-green-500"/>) : (<X className="w-3 h-3 text-red-500"/>));
    return (<AuthLayout title="Create Account" subtitle="Join our community of travelers" activeTab="register" onTabChange={(tab) => tab === 'login' && onSwitchToLogin()} onBack={onBack} onClose={onClose}>
      <form className="space-y-3 max-w-sm mx-auto" onSubmit={handleSubmit}>
        <div className="space-y-1">
          ...
          <div className="relative group">
        ...
        }} placeholder="Enter your full name" required className={`w-full rounded-lg border py-2.5 pl-3 pr-10 text-xs outline-none transition-all placeholder:text-slate-400 ${fieldErrors.name
            ...
            <User className={`absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 transition-colors ${fieldErrors.name ? 'text-red-500' : 'text-slate-400 group-focus-within:text-primary'}`}/>
          </div>
        ...
        <div className="space-y-1">
          ...
          <div className="relative group">
            ...
            ? 'border-red-500 bg-red-50 text-red-900 focus:border-red-500 focus:ring-1 focus:ring-red-500/20 dark:border-red-400 dark:bg-transparent dark:text-red-400'
            : 'border-slate-200 bg-slate-50 text-slate-700 focus:border-primary focus:ring-1 focus:ring-primary/20 dark:border-slate-600 dark:bg-transparent dark:text-slate-200'}`}/>
            <Mail className={`absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 transition-colors ${fieldErrors.email ? 'text-red-500' : 'text-slate-400 group-focus-within:text-primary'}`}/>
          </div>
        ...
        <div className="space-y-1">
          ...
          <div className="relative group">
            ...
            ? 'border-red-500 bg-red-50 text-red-900 focus:border-red-500 focus:ring-1 focus:ring-red-500/20 dark:border-red-400 dark:bg-transparent dark:text-red-400'
            : 'border-slate-200 bg-slate-50 text-slate-700 focus:border-primary focus:ring-1 focus:ring-primary/20 dark:border-slate-600 dark:bg-transparent dark:text-slate-200'}`}/>
            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 transition-colors hover:text-slate-600 dark:text-slate-500">
        ...
        {errorMessage && (<p className="rounded-md border border-rose-400/40 bg-rose-500/10 px-2 py-1.5 text-xs text-rose-600 dark:text-rose-400 max-w-sm mx-auto">
            {errorMessage}
          </p>)}

        <div className="max-w-sm mx-auto">
          <button type="submit" disabled={isSubmitting} className="mt-2 w-full rounded-lg bg-primary py-2.5 text-sm font-bold text-white transition-all active:scale-[0.98] hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-70">
