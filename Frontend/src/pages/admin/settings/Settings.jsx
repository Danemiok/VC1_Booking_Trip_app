...
export const Settings = () => {
    ...
    const tabs = ['General', 'Security', 'Notifications', 'Payment Gateways'];
    const renderActiveSection = () => {
        if (activeTab === 'General') {
            return (<GeneralSettingsSection defaultCurrency={defaultCurrency} onDefaultCurrencyChange={setDefaultCurrency} theme={theme} onThemeChange={setTheme}/>);
        }
    ...
    return (<div className="max-w-5xl w-full mx-auto px-6 py-8">
      ...
      <div className="border-b border-slate-200 dark:border-slate-800 mb-8">
        <div className="flex gap-8 overflow-x-auto scrollbar-hide">
          {tabs.map((tab) => (<button key={tab} onClick={() => setActiveTab(tab)} className={cn("pb-4 px-1 text-sm font-bold whitespace-nowrap transition-all", activeTab === tab
