...
import { Download, Search, Filter, Calendar, ChevronDown, Edit3, UserMinus, LogIn, Lock, Settings as SettingsIcon, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const AuditLogs = () => {
    ...
    const actionOptions = useMemo(() => ['All Actions', ...Array.from(new Set(logs.map((log) => log.action)))], []);
    const statusOptions = useMemo(() => ['All Statuses', ...Array.from(new Set(logs.map((log) => log.status)))], []);
 ...
 <Worksheet ss:Name="Audit Logs">
  <Table>
   ...
   ${rowsXml}
    ...
    return (<div className="flex-1 flex flex-col p-6 lg:p-10 gap-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={handlePrepareReportFile} className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/70 transition-colors">
            <Download size={18}/>
            Prepare Report File
          </button>
          <button onClick={handleExportExcel} className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/70 transition-colors">
            <Download size={18}/>
            Export Excel
          </button>
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-3 lg:items-center">
        <div className="flex-1">
          <div className="relative group">
            ...
            <input className="block w-full h-11 pl-11 pr-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none placeholder:text-slate-400 transition-colors" placeholder="Search by user, action, or IP address..." type="text" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)}/>
          </div>
        </div>
        <div className="flex flex-col sm:flex-row gap-2 w-full lg:w-auto">
          <div className="relative w-full sm:w-auto">
            <Filter size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/>
            <select className="appearance-none h-11 w-full sm:min-w-[180px] rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 pl-9 pr-9 text-sm font-medium text-slate-700 dark:text-slate-300 outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors" value={actionFilter} onChange={(event) => setActionFilter(event.target.value)}>
          ...
          <div className="relative w-full sm:w-auto">
            ...
            <select className="appearance-none h-11 w-full sm:min-w-[180px] rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 pl-9 pr-9 text-sm font-medium text-slate-700 dark:text-slate-300 outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {filteredLogs.map((log, i) => (<tr key={i} className={cn("table-row", log.alert && "bg-red-50/20 dark:bg-red-900/5")}>
                  ...
                  <td className="px-6 py-4">
                    <span className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium", log.status === 'Success' ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700")}>
