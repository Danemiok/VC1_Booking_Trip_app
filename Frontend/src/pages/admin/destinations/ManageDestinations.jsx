    ...
    return (<div className="p-8 space-y-8">
      ...
      <div className="card p-4 flex flex-wrap items-center gap-4">
        <div className="flex-1 min-w-[300px] relative">
          ...
          <input className="input-base pl-10" placeholder="Search destinations by name or region..." type="text" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)}/>
        </div>
        <div className="flex items-center gap-3">
          <select className="select-base min-w-[200px]" value={selectedRegion} onChange={(event) => setSelectedRegion(event.target.value)}>
            {regions.map((region) => (<option key={region} value={region}>
                {region}
              </option>))}
          </select>
          <select className="select-base min-w-[200px]" value={selectedCategory} onChange={(event) => setSelectedCategory(event.target.value)}>
      ...
      <div className="card overflow-hidden">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
            {filteredDestinations.map((dest, i) => (<tr key={i} className="table-row">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={cn("flex items-center gap-1.5 text-sm font-medium", dest.status === 'Active' ? "text-emerald-600" : "text-slate-400")}>
