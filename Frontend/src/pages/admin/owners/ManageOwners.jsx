...
import { Building2, Home, Truck, TrendingUp, Plus, Filter, FileText, Eye, Edit2, Trash2, Search, ChevronLeft, ChevronRight, X } from 'lucide-react';
import { cn } from '../../../utils/utils';
const MetricCard = ({ title, value, trend, icon: Icon }) => (<div className="card p-4">
...
export const OwnerManagement = ({ onViewAllApplications, onViewOwnerDetails }) => {
    ...
    return (<div className="p-8 space-y-8 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <MetricCard title="Total Owners" value="1,248" trend="4.8" icon={Building2}/>
        <MetricCard title="Accommodation Owners" value="892" trend="3.1" icon={Home}/>
        <MetricCard title="Accommodation & Transport" value="356" trend="2.4" icon={Truck}/>
      </div>
      ...
      <div className="card overflow-hidden">
        <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
          ...
          <div className="flex gap-3">
            <button type="button" onClick={onViewAllApplications} className="flex items-center gap-2 px-4 py-2 border border-primary/30 text-primary hover:bg-primary/10 rounded-lg text-sm font-medium transition-colors">
              <FileText size={18}/>
              <span>View All Applications</span>
            </button>
            <button type="button" onClick={() => setIsFilterOpen((prev) => !prev)} className={cn("flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors", isFilterOpen
        ...
        {isFilterOpen && (<div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/40">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <div className="md:col-span-2 relative">
                <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/>
                <input type="text" value={filters.keyword} onChange={(e) => setFilters((prev) => ({ ...prev, keyword: e.target.value }))} placeholder="Search by business or owner name" className="input-base pl-9"/>
              </div>
              <select className="select-base w-full" value={filters.type} onChange={(e) => setFilters((prev) => ({ ...prev, type: e.target.value }))}>
                <option value="All">All Types</option>
                <option value="Accommodation">Accommodation</option>
                <option value="Both">Both</option>
              </select>
              <div className="flex gap-2">
                <select className="select-base w-full" value={filters.status} onChange={(e) => setFilters((prev) => ({ ...prev, status: e.target.value }))}>
                  <option value="All">All Status</option>
                  <option value="Active">Active</option>
                  <option value="Inactive">Inactive</option>
                </select>
                <button type="button" onClick={() => setFilters({ keyword: '', type: 'All', status: 'All' })} className="px-3 py-2 text-xs font-semibold border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors whitespace-nowrap">
                  Reset
                </button>
              </div>
            </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {filteredOwners.map((owner, i) => (<tr key={i} className="table-row">
                  <td className="px-6 py-4 text-sm font-semibold">{owner.business}</td>
                  <td className="px-6 py-4 text-sm">{owner.owner}</td>
                  <td className="px-6 py-4">
                    <span className={cn("px-2.5 py-1 text-[10px] font-bold rounded-full uppercase", owner.type === 'Accommodation' ? "bg-primary/10 text-primary" : "bg-orange-100 text-orange-600")}>
                      {owner.type}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-sm font-medium">{owner.count}</td>
                  <td className="px-6 py-4">
                    <span className={cn("flex items-center gap-1.5 text-sm font-medium", owner.status === 'Active' ? "text-emerald-600" : "text-slate-400")}>
                ...
                openOwnerDetails(owner);
                      ...
                      <button type="button" onClick={(event) => event.stopPropagation()} className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-200 dark:border-slate-800 rounded-lg text-xs hover:text-primary transition-colors   hover:text-primary transition-colors">
                        <Edit2 size={18}/>
                        Edit
                      </button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <form onSubmit={handleCreateOwner} className="card w-full max-w-lg p-6 space-y-5">
              ...
              <div className="space-y-2">
                ...
                <input className="input-base" required value={formData.business} onChange={(e) => setFormData((prev) => ({ ...prev, business: e.target.value }))} placeholder="Enter business name"/>
              </div>

              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-600 dark:text-slate-300">Owner Name</label>
                <input className="input-base" required value={formData.owner} onChange={(e) => setFormData((prev) => ({ ...prev, owner: e.target.value }))} placeholder="Enter owner full name"/>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-slate-600 dark:text-slate-300">Business Type</label>
                  <select className="select-base w-full" value={formData.type} onChange={(e) => setFormData((prev) => ({ ...prev, type: e.target.value }))}>
                ...
                <div className="space-y-2">
                  ...
                  <input type="number" min={1} className="input-base" required value={formData.count} onChange={(e) => setFormData((prev) => ({ ...prev, count: Number(e.target.value) }))}/>
                </div>
              </div>
