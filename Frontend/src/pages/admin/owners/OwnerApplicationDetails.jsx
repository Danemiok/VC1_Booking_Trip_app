...
import { ArrowLeft, Building2, CalendarDays, CheckCircle2, FileText, Mail, MapPin, Phone, ShieldCheck, XCircle, } from 'lucide-react';
import { useParams } from 'react-router-dom';
import { cn } from '../../../utils/utils';
const ownerApplicationDetailsData = [
    {
        id: 'app-1001',
        name: 'Marc Stevens',
...
export const OwnerApplicationDetails = ({ onBack }) => {
    const { applicationId } = useParams();
    const application = ownerApplicationDetailsData.find((item) => item.id === applicationId) ??
        ownerApplicationDetailsData[0];
    const [status, setStatus] = React.useState(application.status);
    const [showApproveModal, setShowApproveModal] = React.useState(false);
    const [approvedAt, setApprovedAt] = React.useState(null);
    React.useEffect(() => {
    ...
    return (<div className="p-8 space-y-6">
      <div>
        <button type="button" onClick={onBack} className="mb-4 inline-flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-primary transition-colors">
          <ArrowLeft size={16}/>
      ...
      <div className="card p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            {approvedAt && (<p className="text-xs text-emerald-600 font-semibold mt-2">Approved on {approvedAt}</p>)}
          </div>
          <span className={cn('px-3 py-1 rounded-full text-xs font-bold uppercase', status === 'Pending' && 'bg-amber-100 text-amber-700', status === 'Approved' && 'bg-emerald-100 text-emerald-700', status === 'Rejected' && 'bg-red-100 text-red-700')}>
            {status}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card p-6 space-y-4">
          ...
          {application.documents.map((doc) => (<div key={doc.file} className="p-3 rounded-xl border border-slate-200 dark:border-slate-800">
          ...
          <div className="pt-2 space-y-2">
            <button type="button" onClick={openApproveModal} disabled={status !== 'Pending'} className={cn("w-full inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-colors", status === 'Pending'
            ? "bg-primary text-white hover:bg-primary/90"
            : "bg-slate-200 text-slate-500 cursor-not-allowed")}>
              <CheckCircle2 size={16}/>
              {status === 'Approved' ? 'Application Approved' : 'Approve Application'}
            </button>
            <button type="button" onClick={() => setStatus('Rejected')} disabled={status === 'Approved'} className={cn("w-full inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-colors", status === 'Approved'
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="card w-full max-w-md p-6 space-y-5">
              ...
              <div className="flex justify-end gap-2">
                <button type="button" onClick={closeApproveModal} className="px-4 py-2 text-sm font-semibold border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  Cancel
                </button>
                <button type="button" onClick={confirmApproveApplication} className="px-4 py-2 text-sm font-semibold bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors">
                  Confirm Approve
                </button>
              </div>
            </div>
