...
export const ViewAllApplications = ({ onBack, onViewDetails }) => {
    const [applications, setApplications] = useState(initialApplications);
    const [selectedApplication, setSelectedApplication] = useState(null);
    const openApproveCard = (application) => {
    ...
    const confirmApprove = () => {
        if (!selectedApplication)
            return;
        setApplications((prev) => prev.map((application) => application.email === selectedApplication.email
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {applications.map((application) => (<tr key={application.id} className="table-row">
                  ...
                  <td className="px-6 py-4">
                    <span className={cn('px-2.5 py-1 text-[10px] font-bold rounded-full uppercase', application.type === 'Accommodation' ? 'bg-primary/10 text-primary' : 'bg-orange-100 text-orange-600')}>
                      {application.type}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={cn('flex items-center gap-1.5 text-xs font-medium', application.status === 'Approved' ? 'text-emerald-600' : 'text-amber-600')}>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2">
                      <button type="button" onClick={() => onViewDetails?.(application.id)} className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-200 dark:border-slate-800 rounded-lg text-xs font-bold hover:text-primary transition-colors">
                        <Eye size={14}/>
                        Details
                      </button>
                      <button type="button" onClick={() => openApproveCard(application)} disabled={application.status === 'Approved'} className={cn('flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors', application.status === 'Approved'
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="card w-full max-w-md p-6 space-y-5">
              ...
              <div className="flex justify-end gap-2">
                <button type="button" onClick={cancelApprove} className="px-4 py-2 text-sm font-semibold border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  Cancel
                </button>
                <button type="button" onClick={confirmApprove} className="px-4 py-2 text-sm font-semibold bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors">
                  Approve
                </button>
              </div>
            </div>
