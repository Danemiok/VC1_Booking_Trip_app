...
export const AdminProfile = ({ onDirtyChange }) => {
    const { updateUser } = useAuth();
    const [isEditingProfile, setIsEditingProfile] = React.useState(false);
    const [savedProfile, setSavedProfile] = React.useState(INITIAL_PROFILE);
    const [profileForm, setProfileForm] = React.useState(INITIAL_PROFILE);
    ...
    const isDirty = isEditingProfile && (profileForm.fullName !== savedProfile.fullName ||
    ...
    if (isLoadingProfile) {
        return (<div className="max-w-6xl w-full mx-auto px-6 md:px-8 py-8">
        <div className="card p-6">
          <p className="text-sm text-slate-500">Loading admin profile...</p>
        </div>
      </div>);
    }
    return (<div className="max-w-6xl w-full mx-auto px-6 md:px-8 py-8 space-y-6">
      {alert && (<div className={`card border ${alertStyles} p-4 flex items-start justify-between gap-3`}>
      ...
      <div className="flex flex-wrap items-start justify-between gap-5">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <button type="button" onClick={handleToggleEdit} className="h-12 px-6 rounded-2xl border border-slate-300/90 dark:border-slate-700 text-slate-700 dark:text-slate-200 text-base font-semibold hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors flex-1 sm:flex-none">
            {isEditingProfile ? 'Cancel Edit' : 'Edit Profile'}
          </button>
          <button type="button" onClick={handleSaveProfile} disabled={isSavingProfile} className="h-12 px-6 rounded-2xl bg-primary text-white text-base font-semibold hover:bg-primary/90 shadow-sm transition-colors flex-1 sm:flex-none disabled:opacity-70 disabled:cursor-not-allowed">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-5">
          <div className="relative w-36 h-36 md:w-40 md:h-40">
            <div className="w-full h-full rounded-full border-[6px] border-white overflow-hidden bg-slate-100 shadow-lg">
              <img src={profileImage || `https://ui-avatars.com/api/?name=${encodeURIComponent(profileForm.fullName || 'Admin')}&background=0D8ABC&color=fff&size=256`} alt="Admin profile" className="w-full h-full object-cover rounded-full"/>
            </div>
            <button type="button" onClick={openPhotoPicker} className={cn("absolute -right-2 -bottom-2 w-12 h-12 rounded-full flex items-center justify-center shadow-md border-4 border-white transition-all bg-slate-200 z-10", isEditingProfile
            ? "text-black hover:bg-slate-300 cursor-pointer"
            : "text-slate-500 cursor-not-allowed")} title="Edit photo" aria-label="Edit profile photo">
              <Camera size={22}/>
            </button>
          </div>
      ...
      <section className="card p-6 md:p-7 rounded-2xl space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="flex flex-col gap-2">
            ...
            <input className="input-base h-12 disabled:opacity-60 disabled:cursor-not-allowed" type="text" value={profileForm.fullName} onChange={(event) => setProfileForm((prev) => ({ ...prev, fullName: event.target.value }))} disabled={!isEditingProfile}/>
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-sm font-semibold text-slate-700 dark:text-slate-200">Email Address</label>
            <input className={cn('input-base h-12 disabled:opacity-60 disabled:cursor-not-allowed', emailError && 'border-red-400 focus:border-red-400 focus:ring-red-400/30')} type="email" value={profileForm.email} onFocus={enableEditModeForEmail} onChange={(event) => {
            enableEditModeForEmail();
            setProfileForm((prev) => ({ ...prev, email: event.target.value }));
        }}/>
            {emailError && (<p className="text-xs font-medium text-red-600 dark:text-red-400">{emailError}</p>)}
          </div>
          <div className="flex flex-col gap-2 md:col-span-2">
            <label className="text-sm font-semibold text-slate-700 dark:text-slate-200">Phone Number</label>
            <input className="input-base h-12 disabled:opacity-60 disabled:cursor-not-allowed" type="tel" value={profileForm.phoneNumber} onChange={(event) => setProfileForm((prev) => ({ ...prev, phoneNumber: event.target.value }))} disabled={!isEditingProfile}/>
          </div>
          <div className="flex flex-col gap-2 md:col-span-2">
            <label className="text-sm font-semibold text-slate-700 dark:text-slate-200">Bio</label>
            <textarea className="input-base min-h-28 resize-none disabled:opacity-60 disabled:cursor-not-allowed" value={profileForm.bio} onChange={(event) => setProfileForm((prev) => ({ ...prev, bio: event.target.value }))} disabled={!isEditingProfile}/>
          </div>
        </div>
      </section>
      ...
      <div className="flex justify-end gap-3 pb-2">
        ...
        <button type="button" onClick={handleSaveProfile} disabled={isSavingProfile} className="h-11 px-8 rounded-lg bg-primary text-white font-semibold hover:bg-primary/90 shadow-md shadow-primary/20 transition-all disabled:opacity-70 disabled:cursor-not-allowed">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-md card p-6">
              ...
              <div className="mt-5 flex justify-end gap-3">
                <button type="button" onClick={dismissDiscardProfileEdit} className="h-10 px-4 rounded-lg border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-semibold hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  Cancel
                </button>
                <button type="button" onClick={confirmDiscardProfileEdit} className="h-10 px-4 rounded-lg bg-amber-500 text-white font-semibold hover:bg-amber-600 transition-colors">
                  Okay
                </button>
              </div>
            </div>
