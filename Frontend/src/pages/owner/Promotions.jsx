...
import { Tag, Plus, Search, Filter, MoreHorizontal, TrendingUp, Users, CheckCircle2, Clock, AlertCircle } from 'lucide-react';
        ...
        const fetchPromotions = async () => {
            try {
                ...
                const transformedPromotions = data.map((p) => {
                    ...
                    const normalizedCategory = linkedDestinations.length && linkedTransports.length
                        ? 'all'
      ...
      <section className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border-transparent rounded-xl text-sm transition-all focus:bg-white dark:focus:bg-slate-900" placeholder="Search campaigns..." type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filteredPromotions.map((campaign) => (<tr key={campaign.id} className="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                  <td className="px-6 py-4">
                    ...
                    {(campaign.linked_destinations?.length || campaign.linked_transports?.length) ? (<p className="text-[10px] text-emerald-600 font-medium mt-1">
