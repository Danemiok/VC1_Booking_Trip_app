...
import { ArrowLeft, Star, MapPin, Users, Wifi, Car, Coffee, Tv, Wind, Dumbbell, Utensils, Edit, Trash2, Plus, Eye } from 'lucide-react';
const PropertyDetail = () => {
    ...
    React.useEffect(() => {
        ...
        const propertyRooms = storedRooms.filter((r) => r.propertyId === property.id);
    ...
    const handleDeleteRoom = (room) => {
        ...
        const propertyRooms = updatedRooms.filter((r) => r.propertyId === property.id);
    ...
    const propertyImages = React.useMemo(() => {
        const gallery = Array.isArray(property?.images)
            ? property.images.filter((image) => typeof image === 'string' && image.trim().length > 0)
            : [];
    ...
    return (<div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="bg-white dark:bg-gray-800 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <button onClick={() => navigate('/destinations')} className="flex items-center text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors">
            ...
            <div className="lg:col-span-2">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <img src={propertyImages[0]} alt={property.name} className="w-full h-96 object-cover rounded-xl"/>
                </div>
                <img src={propertyImages[1]} alt={`${property.name} view 2`} className="w-full h-48 object-cover rounded-xl"/>
                <img src={propertyImages[2]} alt={`${property.name} view 3`} className="w-full h-48 object-cover rounded-xl"/>
              </div>
              {propertyImages.length > 3 && (<p className="mt-3 text-sm font-medium text-gray-500 dark:text-gray-400">
            ...
            <div className="space-y-6">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold text-white ${property.status === 'active' ? 'bg-green-500' : 'bg-yellow-500'}`}>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {rooms.map((room) => (<div key={room.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
              <div className="relative">
                <img src={room.image} alt={room.name} className="w-full h-48 object-cover"/>
                {!room.available && (<div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
              ...
              <div className="p-4">
                ...
                <div className="flex flex-wrap gap-1 mb-4">
                  {(room.amenities || []).slice(0, 3).map((amenity, index) => (<span key={index} className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2 py-1 rounded">
                      {amenity}
                    </span>))}
                  {(room.amenities || []).length > 3 && (<span className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2 py-1 rounded">
                ...
                : 'bg-gray-300 dark:bg-gray-600 text-gray-500 dark:text-gray-400'}`}>
                  ...
                  <button onClick={() => navigate(`/destinations/${property.id}/add-room`, { state: { property, room } })} className="h-10 w-10 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors inline-flex items-center justify-center" title="Edit room" aria-label="Edit room">
                    <Edit size={16}/>
                  </button>
                  <button onClick={() => handleDeleteRoom(room)} className="h-10 w-10 rounded-lg bg-red-600 text-white hover:bg-red-700 transition-colors inline-flex items-center justify-center" title="Delete room" aria-label="Delete room">
                    <Trash2 size={16}/>
                  </button>
                </div>
              </div>
      ...
      {showRoomModal && selectedRoom && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-900 rounded-2xl max-w-2xl w-full mx-4 max-h-[85vh] flex flex-col overflow-hidden">
            <div className="p-6 flex items-start justify-between gap-4 border-b border-gray-100 dark:border-gray-800">
              ...
              <button onClick={() => setShowRoomModal(false)} className="w-10 h-10 rounded-xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white" aria-label="Close">
                ✕
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <img src={selectedRoom.image} alt={selectedRoom.name} className="w-full h-56 object-cover rounded-xl"/>
                <div className="space-y-2">
                  <div className="flex justify-between gap-3">
