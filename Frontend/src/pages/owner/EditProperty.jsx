...
import { ArrowLeft, Upload, CheckCircle2, ChevronRight, Info, Image as ImageIcon, DollarSign, Save, Trash2, X, Plus } from 'lucide-react';
...
const normalizePropertyData = (propertyData) => {
    const imageList = Array.isArray(propertyData?.images)
        ? propertyData.images.filter((image) => typeof image === 'string' && image.trim().length > 0)
        : [];
    const image = resolveImageUrl((typeof propertyData?.image === 'string' && propertyData.image.trim()) || imageList[0] || '') ||
        DEFAULT_IMAGE;
    ...
    return {
        ...
        status: propertyData?.status === 'active' ? 'active' : 'draft',
        totalBookings,
...
const EditProperty = () => {
    ...
    if (isLoading) {
        return (<div className="p-8 max-w-[1000px] mx-auto">
    ...
    return (<div className="p-8 max-w-[1000px] mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowDeleteModal(true)} disabled={isSubmitting || isDeleting} className="flex items-center px-4 py-2 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 transition-colors">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        {submitError && (<div className="mx-8 mt-8 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/30 dark:bg-red-900/20 dark:text-red-300">
            {submitError}
          </div>)}

        {/* Step 1: Basic Info */}
        {step === 1 && (<div className="p-8 space-y-8">
        ...
        {step === 2 && (<div className="p-8 space-y-8">
            <div className="border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl p-12 flex flex-col items-center justify-center text-center hover:bg-blue-50 dark:hover:bg-blue-900/10 hover:border-blue-600/50 transition-all cursor-pointer group">
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button onClick={() => addImage()} className="px-6 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold hover:bg-slate-50 transition-all">
                  Add Image
                </button>
              </div>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {formData.images.map((image, index) => (<div key={index} className="aspect-square relative group">
                  <img src={image} alt={`Property ${index + 1}`} className="w-full h-full object-cover rounded-xl"/>
                  <button onClick={() => removeImage(index)} className="absolute top-2 right-2 p-1 bg-red-600 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity" aria-label="Remove image">
                    <X size={16}/>
                  </button>
                </div>))}
              {newImagePreviews.map((image, index) => (<div key={`new-${index}`} className="aspect-square relative group">
                  <img src={image} alt={`New property ${index + 1}`} className="w-full h-full object-cover rounded-xl"/>
                  <button onClick={() => removeNewImage(index)} className="absolute top-2 right-2 p-1 bg-red-600 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity" aria-label="Remove image">
                    <X size={16}/>
                  </button>
                </div>))}
        ...
        {/* Step 3: Pricing */}
        {step === 3 && (<div className="p-8 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-6">
                <div className="space-y-2">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-8 max-w-md w-full mx-4">
            ...
            <div className="flex gap-3">
              <button onClick={() => {
                setShowDeleteModal(false);
                setDeleteError('');
            }} disabled={isDeleting} className="flex-1 px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg font-medium hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                Cancel
              </button>
              {deleteError && (<div className="mb-0 hidden"/>)}
              <button onClick={handleDelete} disabled={isDeleting} className="flex-1 px-4 py-2 bg-red-600 text-white rounded-lg font-medium hover:bg-red-700 disabled:opacity-50 transition-colors">
                {isDeleting ? 'Deleting...' : 'Delete Property'}
              </button>
            </div>
            {deleteError && (<div className="mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/30 dark:bg-red-900/20 dark:text-red-300">
                {deleteError}
              </div>)}
          </div>
        </div>)}
