...
const Financials = () => {
    ...
    return (<div className="p-8 space-y-8 max-w-[1600px] mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button onClick={() => navigate('/financials/withdraw')} className="px-5 py-2.5 bg-blue-600 text-white rounded-xl flex items-center gap-2 font-bold text-sm hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/20 active:scale-95">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-8">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            { name: 'Wing Bank', account: '**** 3321', type: 'Secondary', icon: CreditCard },
        ].map((method, i) => (<div key={i} className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center justify-between group cursor-pointer hover:border-blue-600/30 transition-all">
                  ...
                  <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full", method.type === 'Primary' ? "bg-blue-600 text-white" : "bg-slate-200 dark:bg-slate-700 text-slate-500")}>
                    {method.type}
                  </span>
                </div>))}
            </div>
