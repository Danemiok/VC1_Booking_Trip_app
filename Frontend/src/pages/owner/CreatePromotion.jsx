...
import { ArrowLeft, Tag, Calendar, CheckCircle2, ChevronRight, Info, Target, Zap, Layout, MapPin, Car, Search } from 'lucide-react';
...
const CreatePromotion = () => {
    ...
    const [promotionType, setPromotionType] = React.useState(null);
    const [serviceCategory, setServiceCategory] = React.useState('hotel');
    ...
    const [linkedDestinations, setLinkedDestinations] = React.useState([]);
    const [linkedTransports, setLinkedTransports] = React.useState([]);
    ...
    const effectiveServiceCategory = linkedDestinations.length > 0 && linkedTransports.length > 0 ? 'all' : serviceCategory;
    ...
    const filteredDestinations = destinations.filter(d => d.name.toLowerCase().includes(searchDestination.toLowerCase()) ||
        d.location.toLowerCase().includes(searchDestination.toLowerCase()));
    const filteredTransports = transports.filter(t => t.name.toLowerCase().includes(searchTransport.toLowerCase()) ||
        t.route.toLowerCase().includes(searchTransport.toLowerCase()));
    const getDiscountSuffix = () => {
        if (promotionType === 'Percentage Discount')
            return '%';
        if (promotionType === 'Fixed Amount Off')
            return '$';
        return '';
    };
    ...
    return (<div className="p-8 max-w-[1000px] mx-auto space-y-8">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        {step === 3 && (<div className="p-8 space-y-6">
            ...
            {/* Linked Destinations */}
            {showDestinationsSection && (<div className="space-y-3">
              ...
              <div className="max-h-48 overflow-y-auto space-y-2 border border-slate-200 dark:border-slate-700 rounded-lg p-2">
                {isLoadingDestinations ? (<p className="text-sm text-slate-500 text-center py-4">Loading destinations...</p>) : filteredDestinations.length === 0 ? (<p className="text-sm text-slate-500 text-center py-4">No destinations found</p>) : (filteredDestinations.map((destination) => (<div key={destination.id} onClick={() => toggleDestination(destination.id)} className={cn("flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors", linkedDestinations.includes(destination.id)
                      ...
                      <div className={cn("w-5 h-5 rounded flex items-center justify-center border", linkedDestinations.includes(destination.id)
        ...
        {step === 4 && (<div className="p-8 space-y-6">
        ...
        {step === 5 && (<div className="p-8 space-y-6">
            ...
            <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 p-6 space-y-4">
              ...
              <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {linkedDestinations.length === 0 && linkedTransports.length === 0 ? (<span className="text-sm text-slate-500">No items selected (promotion will not apply to any items)</span>) : (<>
                      {linkedDestinations.map(id => {
                    const dest = destinations.find(d => d.id === id);
                    return dest ? (<span key={id} className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-100 text-emerald-700 rounded-lg text-xs font-medium">
                            <MapPin size={12}/>
                            {dest.name}
                          </span>) : null;
                })}
