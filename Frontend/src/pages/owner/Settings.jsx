...
import { User, Bell, Shield, Users, CalendarCheck, Wallet, MessageSquare, Tag, BarChart3, Mail, Phone, MapPin, Camera, MoreHorizontal, Plus, Pencil, Check, X } from 'lucide-react';
...
const Settings = () => {
    ...
    const [businessLogo, setBusinessLogo] = React.useState('https://picsum.photos/seed/logo/200/200');
    ...
    const [ownerDraft, setOwnerDraft] = React.useState(ownerInfo);
    React.useEffect(() => {
        if (isEditingOwner)
            return;
    ...
    React.useEffect(() => {
        try {
            localStorage.setItem(SETTINGS_STORAGE_KEY, JSON.stringify({ notificationPrefs, teamMembers, securityPrefs }));
        }
        catch {
            // ignore
        }
    ...
    const toggleNotification = (key) => {
        setNotificationPrefs((prev) => prev.map((p) => (p.key === key ? { ...p, active: !p.active } : p)));
        showToast('Notification settings saved');
    };
    const [isAddMemberOpen, setIsAddMemberOpen] = React.useState(false);
    const [newMemberDraft, setNewMemberDraft] = React.useState({
        name: '',
        role: 'Manager',
        email: '',
        status: 'pending',
    ...
    return (<div className="p-8 max-w-[1200px] mx-auto space-y-8">
      ...
      {toast && (<div className="rounded-2xl border border-emerald-200 dark:border-emerald-900/40 bg-emerald-50 dark:bg-emerald-900/10 px-4 py-3 text-sm font-bold text-emerald-700 dark:text-emerald-300">
          {toast}
        </div>)}
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <main className="flex-1 space-y-8">
          {activeTab === 'profile' && (<div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
              ...
              <div className="p-8 space-y-6">
                <div className="rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                  ...
                  {!isEditingOwner ? (<div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="flex items-center gap-3 p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                        <img alt={ownerInfo.name} className="w-10 h-10 rounded-xl object-cover border border-slate-200 dark:border-slate-700" src={ownerInfo.avatar} referrerPolicy="no-referrer"/>
                    ...
                    </div>) : (<div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {teamMembers.map((member, i) => (<tr key={i} className="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-bold rounded-full", member.status === 'active' ? "bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600" : "bg-amber-50 dark:bg-amber-900/20 text-amber-600")}>
          ...
          {activeTab === 'notifications' && (<div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-8 space-y-8">
              <h4 className="font-bold text-lg">Notification Preferences</h4>
              <div className="space-y-6">
                {notificationPrefs.map((item, i) => (<div key={i} className="flex items-center justify-between">
      ...
      {isAddMemberOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
      ...
      {isPasswordOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
