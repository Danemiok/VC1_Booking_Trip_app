...
import { ArrowLeft, Upload, CheckCircle2, ChevronRight, Info, Image as ImageIcon, Bed, Bath, Users, Wifi, Tv, Wind, Coffee, Car, Dumbbell, Utensils, Plus, X } from 'lucide-react';
...
const AddRoom = () => {
    ...
    const validateStep = (currentStep) => {
        ...
        if (currentStep === 1) {
            ...
            if (!formData.beds)
                newErrors.beds = 'Bed configuration is required';
    ...
    const handleSubmit = () => {
        ...
        const updatedRooms = roomToEdit
            ? existingRooms.map((r) => (r.id === roomToEdit.id ? { ...r, ...newRoom } : r))
    ...
    return (<div className="p-8 max-w-[1000px] mx-auto space-y-8">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3">
                    <input type="checkbox" checked={formData.available} onChange={(e) => setFormData({ ...formData, available: e.target.checked })} className="w-4 h-4 rounded"/>
        ...
        {step === 2 && (<div className="p-8 space-y-8">
            ...
            {formData.images.length > 0 && (<div className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {formData.images.map((image, index) => (<div key={index} className="aspect-square relative group">
                      <img src={image} alt={`Room ${index + 1}`} className="w-full h-full object-cover rounded-xl border-2 border-slate-200 dark:border-slate-700"/>
                      <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 transition-all rounded-xl flex items-center justify-center">
                        <button onClick={() => removeImage(index)} className="opacity-0 group-hover:opacity-100 transition-opacity p-2 bg-red-600 text-white rounded-full hover:bg-red-700 transform scale-90 group-hover:scale-100 transition-transform">
                          <X size={16}/>
                        </button>
                      </div>
                ...
                {formData.images.length < 3 && (<div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 rounded-xl p-4">
