...
import { ArrowLeft, Mail, Search, Send, MessageCircle, Phone, Video, MoreVertical, Info } from 'lucide-react';
...
export default function OwnerMessagesPage() {
    ...
    const recentCustomers = useMemo(() => {
        ...
        [...messages]
            ...
            .forEach((message) => {
            const otherId = customerMap.has(String(message.sender_id)) ? String(message.sender_id) : String(message.receiver_id);
            const customer = customerMap.get(otherId);
            if (!customer || seen.has(String(customer.id)))
                return;
    ...
    const searchResults = useMemo(() => {
        ...
        if (!q)
            return recentCustomers.length > 0 ? recentCustomers : customers;
    ...
    const conversationMessages = useMemo(() => {
        ...
        return messages
            .filter((message) => String(message.sender_id) === id || String(message.receiver_id) === id)
    ...
    const lookupCustomerByEmail = async (email) => {
        ...
        setLookupError(null);
        try {
            const payload = await apiRequest('/owner/customers/search?email=' + encodeURIComponent(email));
    ...
    async function openConversation(customer) {
        ...
        setError(null);
        try {
            const payload = await apiRequest(`/messages/${customer.id}`);
            const list = Array.isArray(payload) ? payload : payload.data || [];
            setMessages((current) => {
                const rest = current.filter((message) => String(message.sender_id) !== String(customer.id) &&
    ...
    async function sendMessage(event) {
        ...
        if (!selectedCustomer || !draft.trim() || sending)
            return;
        setSending(true);
        setError(null);
        try {
            ...
            setDraft('');
            const payload = await apiRequest(`/messages/${selectedCustomer.id}`);
            const list = Array.isArray(payload) ? payload : payload.data || [];
            setMessages((current) => {
                const rest = current.filter((message) => String(message.sender_id) !== String(selectedCustomer.id) &&
        ...
        <div className="grid flex-1 grid-cols-1 overflow-hidden lg:grid-cols-[360px_1fr]">
          <aside className="border-r border-slate-200 bg-white">
            ...
            <div className="max-h-[calc(100vh-175px)] overflow-y-auto">
              {loading && (<div className="p-6 text-sm text-slate-500">Loading conversations...</div>)}

              {!loading && lookupLoading && (<div className="p-6 text-sm text-slate-500">Looking up customer by email...</div>)}

              {!loading && !lookupLoading && lookupError && (<div className="p-6 text-sm text-slate-500 text-rose-500">
                  {lookupError}
                </div>)}

              {!loading && !lookupLoading && !lookupError && searchResults.length === 0 && (<div className="p-6 text-sm text-slate-500">
            ...
            return (<button key={String(user.id)} type="button" onClick={() => void openConversation(user)} className={`flex w-full items-start gap-3 border-b border-slate-100 px-4 py-4 text-left transition ${isActive ? 'bg-emerald-50' : 'hover:bg-slate-50'}`}>
              ...
              {!loading && recentCustomers.length > 0 && search.trim() === '' && (<div className="px-4 py-3 text-xs uppercase tracking-[0.2em] text-slate-400">
                  Recent customers
                </div>)}
            </div>
          </aside>
          ...
          <main className="flex min-h-0 flex-col bg-slate-50 relative overflow-hidden">
            {selectedCustomer ? (<div className="flex h-full flex-col overflow-hidden relative">
                ...
                <div className="relative flex flex-1 overflow-hidden">
                  ...
                  <div className="flex-1 overflow-y-auto px-6 py-8" style={{
                ...
                backgroundImage: `url("data:image/svg+xml,%3Csvg width='100' height='100' viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M11 18c3.866 0 7-3.134 7-7s-3.134-7-7-7-7 3.134-7 7 3.134 7 7 7zm48 25c3.866 0 7-3.134 7-7s-3.134-7-7-7-7 3.134-7 7 3.134 7 7 7zm-43-7c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zm63 31c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zM34 90c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zm56-76c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zM12 86c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zm66 3c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zm-40-39c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zm14-26c.552 0 1-.448 1-1s-.448-1-1-1-1 .448-1 1 .448 1 1 1zm-8 48c.552 0 1-.448 1-1s-.448-1-1-1-1 .448-1 1 .448 1 1 1zm-10-10c.552 0 1-.448 1-1s-.448-1-1-1-1 .448-1 1 .448 1 1 1zm20-20c.552 0 1-.448 1-1s-.448-1-1-1-1 .448-1 1 .448 1 1 1zm4-4c.552 0 1-.448 1-1s-.448-1-1-1-1 .448-1 1 .448 1 1 1zm-12 12c.552 0 1-.448 1-1s-.448-1-1-1-1 .448-1 1 .448 1 1 1zm-4-4c.552 0 1-.448 1-1s-.448-1-1-1-1 .448-1 1 .448 1 1 1zM42 70c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zm-8-2c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zm-4-4c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zm12 12c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zm4 4c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zm-12-12c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zm-4-4c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2z' fill='%239C92AC' fill-opacity='0.1' fill-rule='evenodd'/%3E%3C/svg%3E")`,
                    ...
                    {error && (<div className="mb-4 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700 shadow-sm">
                        {error}
                      </div>)}
