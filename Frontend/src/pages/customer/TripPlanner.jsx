...
import { Calendar, MapPin, Plus, Trash2, Save, Hotel, Car, Activity, ChevronRight, ArrowRight, Search, ChevronLeft, ChevronRight as ChevronRightIcon, Users } from 'lucide-react';
import { format, parse, startOfMonth, endOfMonth, startOfWeek, endOfWeek, eachDayOfInterval, isSameMonth, isSameDay, isToday, addMonths, subMonths, isBefore, isAfter } from 'date-fns';
...
const parseDateFromString = (value, fallbackYear) => {
...
const getDateRangeFromTripData = (tripData) => {
...
export const TripPlanner = ({ tripData, setTripData, selectedActivityIds, setSelectedActivityIds, onBack, onHotelClick, onExploreHotel, onRentalClick, onActivitiesClick, onProceedToBooking }) => {
    ...
    const fallbackDestination = tripData.destination?.name ||
        tripData.hotel.location.split(',')[0] ||
        DESTINATION_OPTIONS[0].name;
    const [location, setLocation] = useState(fallbackDestination);
    const activeDestinationOption = DESTINATION_OPTIONS.find((dest) => dest.name.toLowerCase() === location.toLowerCase()) ||
    ...
    const baseTotal = (hotelSelected ? tripData.hotel.price : 0) +
        (rentalSelected ? tripData.rental.price : 0) +
        (activitiesSelected ? selectedActivitiesTotal : 0);
    const promotion = tripData?.promotion;
    ...
    const applyDestination = (nextLocation) => {
        const normalizedLocation = nextLocation.trim();
        if (!normalizedLocation)
            return;
        const matchedDestination = DESTINATION_OPTIONS.find((destination) => destination.name.toLowerCase() === normalizedLocation.toLowerCase());
      ...
      <div className="max-w-7xl mx-auto">
        <header className="relative mb-12 rounded-[32px] overflow-visible p-16 md:p-24 min-h-[400px] flex items-end group">
          <div className="absolute inset-0 rounded-[32px] overflow-hidden">
            <img src="https://images.unsplash.com/photo-1528127269322-539801943592?auto=format&fit=crop&q=80&w=2000" alt="Siem Reap Banner" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" referrerPolicy="no-referrer"/>
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-8 w-full">
            ...
            setShowLocationPicker(false);
                  ...
                  <AnimatePresence>
                    {showDatePicker && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute top-full left-0 mt-4 w-[95vw] md:w-[720px] bg-slate-900/95 backdrop-blur-xl rounded-[2.5rem] shadow-2xl border border-white/10 p-8 md:p-10 z-[100]">
            ...
            setShowDatePicker(false);
                  ...
                  <AnimatePresence>
                    {showLocationPicker && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute top-full left-0 mt-4 w-80 bg-slate-900/95 backdrop-blur-xl rounded-[2rem] shadow-2xl border border-white/10 p-6 z-[100]">
                        <div className="relative mb-4">
                          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40"/>
                          <input type="text" value={location} onChange={(e) => setLocation(e.target.value)} onKeyDown={(e) => {
                if (e.key === 'Enter') {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <section>
              ...
              {tripData.rental.isBooked ? (<div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-6 border border-slate-100 dark:border-slate-800 shadow-sm flex gap-6 items-center">
                  <div className="w-24 h-24 rounded-2xl overflow-hidden flex-shrink-0">
            ...
            <section>
              ...
              <div className="space-y-4">
                {selectedActivities.length > 0 ? (selectedActivities.map((activity) => (<motion.div layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} key={activity.id} className="bg-white dark:bg-slate-900 rounded-[2rem] p-4 border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4 group">
                      ...
                      <div className="text-right pr-4">
                        ...
                        <button onClick={() => removeActivity(activity.id)} className="text-[10px] font-bold text-red-500 uppercase tracking-widest hover:text-red-600 flex items-center gap-1 ml-auto">
                          <Trash2 className="w-3 h-3"/> Remove
                        </button>
                      </div>
                    </motion.div>))) : (<div className="w-full py-12 bg-slate-50/50 dark:bg-slate-800/50 rounded-[2.5rem] border border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center text-slate-400">
                    <Activity className="w-8 h-8 mb-2 opacity-20"/>
                    <p className="text-sm font-medium">No activities selected</p>
                  </div>)}
              </div>

              {suggestedActivities.length > 0 && (<div className="mt-12">
                  <h4 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-6">Suggested for you</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {suggestedActivities.map((activity) => (<div key={activity.id} className="bg-white dark:bg-slate-900 rounded-3xl p-4 border border-slate-100 dark:border-slate-800 shadow-sm group">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-slate-900 dark:bg-white rounded-[2.5rem] p-8 text-white dark:text-slate-900 shadow-2xl sticky top-32">
              ...
              <div className="space-y-6 mb-8">
                ...
                {promotion && (<div className="rounded-2xl bg-white/5 dark:bg-slate-100/70 border border-white/10 dark:border-slate-200/70 p-4">
                ...
                return next;
                    ...
                    {promoDiscountAmount > 0 && (<div className="mt-3 flex items-center justify-between">
                ...
                <div className="pt-6 border-t border-white/10 dark:border-slate-100 space-y-3">
                  ...
                  {promoDiscountAmount > 0 && (<div className="flex justify-between items-center">
