...
import { Search, MapPin, Calendar, Car, Home, Filter, Sparkles, Star, Users, Fuel, Gauge, ChevronLeft, ChevronRight, ChevronDown, Heart, ArrowRight } from 'lucide-react';
...
const parsePromotionDiscount = (promotion, basePrice) => {
...
const mapTransportToVehicle = (transport) => {
    ...
    const originalPrice = toNumericValue(transport?.original_price ?? transport?.price_per_km ?? transport?.price ?? pricePerKm);
...
export const Rentals = ({ onBack, onSelectVehicle }) => {
    ...
    const toggleVehicleClass = (vehicleClass) => {
        setSelectedVehicleClasses((prev) => prev.includes(vehicleClass)
    ...
    const filteredVehicles = vehicles.filter((vehicle) => {
        if (vehicle.price > priceRange)
            return false;
        if (showInstantOnly && !vehicle.instantBook)
            return false;
        if (selectedVehicleClasses.length > 0) {
            const vehicleType = normalizeSearchText(vehicle.type || '');
            const matchesClass = selectedVehicleClasses.some((vehicleClass) => vehicleType.includes(normalizeSearchText(vehicleClass)));
            if (!matchesClass)
                return false;
        }
        if (queryTokens.length === 0)
            return true;
        const searchableText = normalizeSearchText([vehicle.name, vehicle.type, vehicle.transmission || '', vehicle.engine || '', vehicle.style || ''].join(' '));
        return queryTokens.every((token) => searchableText.includes(token));
    });
    const filteredStays = stays.filter((stay) => {
        if (stay.price > priceRange)
            return false;
        if (showInstantOnly && !stay.instantBook)
            return false;
        if (queryTokens.length === 0)
            return true;
    ...
    return (<div className="min-h-screen bg-[#FDFDFD] dark:bg-slate-950 pt-20 pb-20">
      {/* Hero Section */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-6">
          <button onClick={onBack} className="inline-flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors">
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-2 w-full">
            <div className="flex items-center gap-3 px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-2xl transition-colors cursor-pointer group">
              ...
              <div className="flex-1">
                ...
                <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={activeTab === 'vehicles' ? 'Search cars...' : 'Search stays...'} className="bg-transparent border-none p-0 text-sm font-bold text-slate-900 dark:text-white placeholder:text-slate-300 focus:ring-0 w-full"/>
              </div>
            </div>
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div id="rental-results" className="flex-1">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex items-center gap-4">
                ...
                <label className="relative">
                  <select value={sortBy} onChange={(event) => setSortBy(event.target.value)} className="appearance-none text-xs font-bold text-slate-900 dark:text-white bg-white dark:bg-slate-900 pl-4 pr-8 py-2 rounded-xl border border-slate-100 dark:border-slate-800">
            ...
            {transportLoading && (<div className="mb-4 rounded-3xl border border-slate-200 bg-white px-6 py-4 text-sm text-slate-500 shadow-sm dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
                Loading transport services...
              </div>)}
            {!transportLoading && transportError && (<div className="mb-4 rounded-3xl border border-red-200 bg-red-50 px-6 py-4 text-sm text-red-700 shadow-sm dark:border-red-800 dark:bg-red-900/20 dark:text-red-300">
                {transportError}
              </div>)}

            {paginatedItems.length === 0 ? (<div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-10 text-center">
                ...
                {activeTab === 'vehicles' ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    return (<motion.div key={car.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 overflow-hidden group hover:shadow-2xl hover:shadow-blue-900/5 transition-all duration-500">
                          ...
                          <div className="p-6">
                            ...
                            {badgeLabel && (<div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-emerald-600">
                            ...
                            {car.insurance && (<div className="flex items-center gap-2 mb-4">
                    ...
                    {paginatedItems.map((stay) => (<motion.div key={stay.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 overflow-hidden group hover:shadow-2xl hover:shadow-blue-900/5 transition-all duration-500">
                        ...
                        <div className="p-6">
                          ...
                          <button onClick={() => onSelectVehicle?.({
                        ...stay,
                        type: 'Vacation Stay',
                        transmission: 'Home'
                ...
                {shouldShowPagination && (<div className="flex items-center justify-center gap-4 mt-12">
                    <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="p-2 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                      <ChevronLeft className="w-5 h-5"/>
                    </button>
                    <div className="flex items-center gap-2">
                      {visiblePages.map((page, index) => (<button key={`${page}-${index}`} onClick={() => typeof page === 'number' && handlePageChange(page)} disabled={page === '...'} className={`w-10 h-10 rounded-xl text-xs font-bold transition-all ${page === currentPage
                        ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900'
                        : 'text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'} ${page === '...' ? 'pointer-events-none' : ''}`}>
                          {page}
                        </button>))}
                    </div>
                    <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="p-2 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
        ...
        {/* Exclusive Stays */}
        {activeTab === 'vehicles' && (<div className="mt-32">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {stays.map((stay) => (<motion.div key={stay.id} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="group cursor-pointer">
                <div className="relative aspect-[4/5] rounded-[2.5rem] overflow-hidden mb-6">
                  <img src={stay.image} alt={stay.name} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" referrerPolicy="no-referrer"/>
