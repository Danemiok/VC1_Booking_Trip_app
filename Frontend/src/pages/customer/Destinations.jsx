import { BedDouble, Calendar, CheckCircle2, ChevronDown, Coffee, Dumbbell, Filter, Heart, MapPin, Search, SlidersHorizontal, Sparkles, Star, Users, Waves, Wifi, X, } from 'lucide-react';
...
export default function Destinations({ onOpenMessages }) {
    ...
    const loadDestinations = React.useCallback(async () => {
        ...
        try {
            ...
            const runtimeMessage = typeof error?.message === 'string' ? error.message.trim() : '';
    ...
    const areaOptions = React.useMemo(() => Array.from(new Set(destinations
        .map((destination) => String(destination.location || '').split(',')[0]?.trim())
        .filter(Boolean))).slice(0, 4), [destinations]);
    const amenityOptions = React.useMemo(() => {
        const uniqueAmenities = Array.from(new Set(destinations.flatMap((destination) => Array.isArray(destination.amenities) ? destination.amenities.filter(Boolean) : [])));
    ...
    const filteredDestinations = React.useMemo(() => {
        ...
        return destinations.filter((destination) => {
            if (search) {
                const matchesSearch = destination.name.toLowerCase().includes(search) ||
                    destination.location.toLowerCase().includes(search) ||
                    destination.type.toLowerCase().includes(search);
                if (!matchesSearch)
                    return false;
            }
            if ((Number(destination.price) || 0) > priceRange)
                return false;
            if (selectedStars !== null && (Number(destination.rating) || 0) < selectedStars)
                return false;
            ...
            if (selectedAmenities.length > 0) {
                ...
                const normalizedAmenities = destinationAmenities.map((amenity) => amenity.toLowerCase());
    ...
    return (<section className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="min-w-0">
          <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex items-center gap-2 self-start rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm shadow-sm dark:border-slate-800 dark:bg-slate-900 lg:self-auto">
              ...
              <select value={sortBy} onChange={(event) => setSortBy(event.target.value)} className="border-none bg-transparent pr-5 text-sm font-semibold text-slate-900 outline-none focus:ring-0 dark:text-slate-100">
          ...
          {loadError && (<div className="mb-6 flex flex-col gap-3 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/30 dark:bg-red-900/20 dark:text-red-300 md:flex-row md:items-center md:justify-between">
              <span>{loadError}</span>
              <button onClick={() => void loadDestinations()} className="rounded-lg bg-red-600 px-4 py-2 font-medium text-white transition hover:bg-red-700">
                Retry
              </button>
            </div>)}

          {isLoading ? (<div id="destination-results" className="rounded-3xl border border-slate-200 bg-white px-6 py-16 text-center text-slate-500 shadow-sm dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
              Loading destinations...
            </div>) : visibleDestinations.length > 0 ? (<div id="destination-results" className="space-y-4">
                    ...
                    <div className="flex flex-col lg:flex-row">
                      <div className="relative lg:w-[43%]">
                        ...
                        if (displayImages.length > 1) {
                            return (<div className="grid h-60 grid-cols-2 grid-rows-2 gap-1 overflow-hidden lg:h-full">
                                {displayImages.map((image, index) => (<img key={`${destination.id}-${index}-${image}`} src={image} alt={`${destination.name} photo ${index + 1}`} className={`h-full w-full object-cover ${index === 0 ? 'row-span-2' : ''}`} referrerPolicy="no-referrer"/>))}
                              </div>);
                        }
                      ...
                      <div className="flex flex-1 flex-col justify-between p-5">
                        <div>
                          <div className="flex flex-col gap-4 border-b border-slate-100 pb-4 dark:border-slate-800 md:flex-row md:items-start md:justify-between">
                            <div>
                              <div className="mb-2 flex items-center gap-1.5 text-amber-400">
                                {Array.from({ length: 5 }, (_, starIndex) => (<Star key={starIndex} className={`h-3.5 w-3.5 ${starIndex < Math.round(destination.rating) ? 'fill-current' : ''}`}/>))}
                              ...
                              {destination.owner ? (<p className="mt-2 text-xs font-semibold text-slate-600 dark:text-slate-300">
                          ...
                          <div className="mt-4 flex flex-wrap gap-2">
                            {highlightAmenities.map((amenity, amenityIndex) => (<span key={`${destination.id}-${amenity}`} className="inline-flex items-center gap-1.5 rounded-md border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-medium text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
                          ...
                          {note ? (<div className={`mt-4 text-xs font-medium ${itemIndex % 3 === 2 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400'}`}>
                        ...
                        <div className="mt-5 flex flex-col gap-3 border-t border-slate-100 pt-4 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            <button type="button" onClick={() => openChatWithOwner(destination.owner)} className="rounded-xl px-6 py-3 text-sm font-semibold bg-emerald-600 text-white transition-colors hover:bg-emerald-700">
                              Chat with Owner
                            </button>
            ...
            </div>) : (<div id="destination-results" className="rounded-3xl border border-slate-200 bg-white px-6 py-16 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      {isLoading ? (<div className="rounded-2xl border border-slate-200 bg-white px-6 py-16 text-center text-slate-500 shadow-sm dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
          Loading destinations...
        </div>) : filteredDestinations.length > 0 ? (<div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
          {filteredDestinations.map((destination) => (<article key={destination.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900">
              <div className="relative">
                {(() => {
                    ...
                    const displayImages = [
                        ...
                        .slice(0, 4);
                    if (displayImages.length > 1) {
                        return (<div className="grid h-56 grid-cols-2 grid-rows-2 gap-1 overflow-hidden">
                        {displayImages.map((image, index) => (<img key={`${destination.id}-${index}-${image}`} src={image} alt={`${destination.name} photo ${index + 1}`} className={`h-full w-full object-cover ${index === 0 ? 'row-span-2' : ''}`} referrerPolicy="no-referrer"/>))}
                      </div>);
                    }
                    return (<img src={displayImages[0] || destination.image} alt={destination.name} className="h-56 w-full object-cover" referrerPolicy="no-referrer"/>);
                })()}
                <div className={`absolute left-4 top-4 rounded-full px-3 py-1 text-xs font-semibold text-white ${destination.status === 'active' ? 'bg-emerald-500' : 'bg-amber-500'}`}>
              ...
              <div className="space-y-4 p-5">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    {destination.has_promotion && destination.promotion && (<div className="flex flex-col gap-1">
                        <span className="inline-block rounded-full bg-red-100 px-2 py-1 text-xs font-semibold text-red-600 dark:bg-red-900/30 dark:text-red-300">
                          {destination.discount_percentage}% OFF
                        </span>
                        {destination.promotion.expiry && (<span className="text-xs text-slate-500 dark:text-slate-400">
                            Expires: {new Date(destination.promotion.expiry).toLocaleDateString()}
                          </span>)}
                      </div>)}
                  </div>
        ...
        </div>) : (<div className="rounded-2xl border border-slate-200 bg-white px-6 py-16 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <h2 className="text-xl font-semibold text-slate-900 dark:text-slate-100">No destinations found</h2>
...
const mapDestinationToHotel = (destination) => {
    ...
    const galleryImage = (Array.isArray(destination?.images) && destination.images.find((img) => String(img || '').trim())) ?? '';
    const resolvedImage = String(destination?.image || galleryImage || '')
...
const getOpenStreetMapSearchUrl = (query) => `https://www.openstreetmap.org/search?query=${encodeURIComponent(query)}`;
const getOpenStreetMapEmbedUrl = (lat, lng, zoom = 11) => {
...
const getOpenStreetMapEmbedUrlForHotel = (hotel) => {
    if (!hotel)
        return getOpenStreetMapEmbedUrl(DEFAULT_SIDEBAR_MAP_CENTER.lat, DEFAULT_SIDEBAR_MAP_CENTER.lng);
...
const getHotelArea = (location) => String(location || '')
    .split(',')[0]
    ?.trim() || 'Central District';
const getHotelCountry = (location) => {
...
const getAmenityMeta = (amenity) => {
    ...
    if (normalizedAmenity.includes('breakfast') ||
...
const DEFAULT_HOTEL_BANNER_IMAGE = 'https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&q=80&w=2000';
...
const parseHotelCreatedAt = (value) => {
    if (!value)
        return 0;
    const parsed = new Date(value);
    const time = parsed.getTime();
...
export const Hotels = ({ tripData, browseDestination, onBack, onSelectHotel, setTripData }) => {
    const [priceRange, setPriceRange] = useState(2000);
    const [searchQuery, setSearchQuery] = useState(() => String(browseDestination?.location || browseDestination?.name || '').trim());
    const [mapLocationQuery, setMapLocationQuery] = useState(() => String(browseDestination?.location || browseDestination?.name || '').trim());
    ...
    const destinationPickerRef = React.useRef(null);
    const stayDatePickerRef = React.useRef(null);
    const t = (key) => {
        const en = {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                const normalized = (Array.isArray(data) ? data : []).map(mapDestinationToHotel).map((hotel) => {
    ...
    useEffect(() => {
        const destinationQuery = String(browseDestination?.location ||
    ...
    const tripNights = checkInDate && checkOutDate
        ? Math.max(1, Math.ceil((new Date(`${checkOutDate}T00:00:00`).getTime() - new Date(`${checkInDate}T00:00:00`).getTime()) /
            (1000 * 60 * 60 * 24)))
        : getTripNights(tripData);
    const stayWindowLabel = checkInDate && checkOutDate
    ...
    const areaOptions = React.useMemo(() => Array.from(new Set(hotels
        .map((hotel) => getHotelArea(hotel.location))
        .filter(Boolean))).slice(0, 4), [hotels]);
    const amenityOptions = React.useMemo(() => Array.from(new Set(hotels.flatMap((hotel) => (Array.isArray(hotel.amenities) ? hotel.amenities.filter(Boolean) : [])))).slice(0, 4), [hotels]);
    useEffect(() => {
        setPriceRange((previous) => Math.min(Math.max(previous, 100), priceCeiling));
    }, [priceCeiling]);
    const destinationSearchIndex = React.useMemo(() => {
        const byArea = new Map();
        const byExactLocation = new Map();
    ...
    const matchesHotelQuery = React.useCallback((hotel) => {
        ...
        const searchableText = normalizeSearchText([hotel.name, hotel.location, getHotelArea(hotel.location), relatedDestinationText].join(' '));
        return queryTokens.every((token) => searchableText.includes(token));
    }, [destinationSearchIndex, queryTokens]);
    const matchesCurrentFilters = React.useCallback((hotel) => {
        if (parsePrice(hotel.price) > priceRange)
            return false;
        if (selectedStars !== null) {
            const hotelRating = typeof hotel.rating === 'number' ? hotel.rating : parseFloat(String(hotel.rating)) || 0;
            if (hotelRating < selectedStars)
                return false;
        }
        if (selectedAreas.length > 0) {
        ...
        if (selectedAmenities.length > 0) {
            ...
            const matchesAmenities = selectedAmenities.every((amenity) => hotelAmenities.includes(normalizeSearchText(amenity)));
            if (!matchesAmenities)
                return false;
        }
    ...
    const sortedHotels = React.useMemo(() => {
        ...
        items.sort((left, right) => {
            if (sortBy === 'newest')
                return parseHotelCreatedAt(right.created_at) - parseHotelCreatedAt(left.created_at);
            if (sortBy === 'price-low')
    ...
    const regionLabel = React.useMemo(() => String(browseDestination?.name ||
        browseDestination?.location ||
        selectedAreas[0] ||
        getHotelArea(sortedHotels[0]?.location || hotels[0]?.location))
        .split(',')[0]
        ?.trim() || 'Cambodia', [browseDestination?.location, browseDestination?.name, hotels, selectedAreas, sortedHotels]);
    const countryLabel = React.useMemo(() => getHotelCountry(String(browseDestination?.location || sortedHotels[0]?.location || hotels[0]?.location || 'Cambodia')), [browseDestination?.location, hotels, sortedHotels]);
    ...
    const areaCoordinatesByLabel = React.useMemo(() => {
        const map = new Map();
        const addCoordinate = (locationValue, latitudeValue, longitudeValue, aliases = []) => {
    ...
    const selectedMapLocation = React.useMemo(() => {
        ...
        for (const [areaKey, coordinate] of areaCoordinatesByLabel.entries()) {
            if (areaKey.includes(normalizedSearch) ||
    ...
    const visibleDestinationPickerOptions = React.useMemo(() => {
        ...
        if (!normalizedQuery)
            return destinationPickerOptions;
        return destinationPickerOptions.filter((option) => normalizeSearchText(option).includes(normalizedQuery));
    }, [destinationPickerOptions, destinationPickerQuery]);
    const applyDestinationFromPicker = (value) => {
    ...
    const openHotelWithSelection = (hotel) => {
    ...
    return (<div className="min-h-screen bg-white pb-16 pt-8 dark:bg-slate-950">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="rounded-[2rem] bg-[#f5f7fb] p-4 shadow-[0_24px_80px_-48px_rgba(15,23,42,0.45)] dark:bg-slate-900/70 sm:p-6 lg:p-8">
          ...
          <section className="relative z-30 mb-7 overflow-visible rounded-[1.8rem] border border-slate-200 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.5)] dark:border-slate-800">
        ...
        }} list={HOTEL_SEARCH_DATALIST_ID} placeholder="Search by location" className="w-full appearance-none border-0 bg-transparent p-0 text-sm font-semibold text-slate-900 outline-none ring-0 shadow-none placeholder:text-slate-400 focus:border-0 focus:outline-none focus:ring-0 focus:shadow-none focus-visible:outline-none focus-visible:ring-0 dark:text-white"/>
                      <datalist id={HOTEL_SEARCH_DATALIST_ID}>
                        {destinationPickerOptions.map((option) => (<option key={`search-${option}`} value={option}/>))}
                      </datalist>
                    </div>
                  </label>

                  <div ref={destinationPickerRef} className="relative min-w-0">
                    <button type="button" aria-expanded={isDestinationPickerOpen} aria-controls="destination-picker-menu" onClick={() => {
            ...
            setIsDestinationPickerOpen((previous) => !previous);
                      ...
                      <ChevronDown className={`h-4 w-4 shrink-0 text-slate-400 transition-transform ${isDestinationPickerOpen ? 'rotate-180' : ''}`}/>
                    </button>
            ...
            }} placeholder={heroLocationLabel} className="w-full appearance-none border-0 bg-transparent p-0 text-sm font-medium text-white outline-none ring-0 shadow-none placeholder:text-slate-400 focus:border-0 focus:outline-none focus:ring-0 focus:shadow-none focus-visible:outline-none focus-visible:ring-0"/>
                        </div>
                ...
                !visibleDestinationPickerOptions.some((option) => normalizeSearchText(option) === normalizeSearchText(destinationPickerQuery)) ? (<button type="button" onClick={() => applyDestinationFromPicker(destinationPickerQuery)} className="flex w-full items-center gap-2.5 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-2 text-left text-sm text-emerald-100 transition-colors hover:bg-emerald-500/20">
                    ...
                    : 'text-slate-300 hover:bg-slate-800/80 hover:text-white'}`}>
                          ...
                          {visibleDestinationPickerOptions.length === 0 ? (<p className="px-2.5 py-2 text-xs text-slate-500">No destinations found.</p>) : null}
                        </div>

                        <button type="button" onClick={() => applyDestinationFromPicker()} className="mt-2 w-full rounded-xl bg-white px-3 py-2.5 text-sm font-semibold uppercase tracking-[0.12em] text-slate-900 transition-colors hover:bg-slate-100">
                          Apply Destination
                        </button>
                      </div>) : null}
                  </div>
                  ...
                  <div ref={stayDatePickerRef} className="relative min-w-0">
                    ...
                    {isStayDatePickerOpen ? (<div className="absolute left-0 top-[calc(100%+0.5rem)] z-[70] w-[320px] rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl dark:border-slate-700 dark:bg-slate-950">
                ...
                setStayDateError('');
                        ...
                        {stayDateError ? (<p className="mt-3 text-xs font-medium text-red-500">{stayDateError}</p>) : (<p className="mt-3 text-xs text-slate-500 dark:text-slate-400">
                ...
                <div className="space-y-6">
                  <section className="rounded-[1rem] border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900/70">
                    ...
                    <div className="mt-2 overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700">
                      <iframe title={`OpenStreetMap for ${selectedMapDisplay.location}`} src={selectedMapEmbedUrl} className="h-[190px] w-full border-0" loading="lazy" referrerPolicy="no-referrer-when-downgrade"/>
                    </div>
            ...
            <div id="hotel-results" className="space-y-5">
              <div className="flex flex-col gap-4 rounded-[1.6rem] border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950 md:flex-row md:items-start md:justify-between">
                ...
                <label className="flex items-center gap-3 self-start rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm dark:border-slate-800 dark:bg-slate-900">
                  ...
                  <select value={sortBy} onChange={(event) => setSortBy(event.target.value)} className="bg-transparent font-semibold text-slate-900 outline-none dark:text-white">
                    <option value="newest">Newest First</option>
              ...
              <div className="space-y-4">
                ...
                {hotelsError && !hotelsLoading && (<div className="rounded-[1.6rem] border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/30 dark:bg-red-900/20 dark:text-red-300">
                    {hotelsError}
                  </div>)}

                {!hotelsLoading && !hotelsError && sortedHotels.length > 0 && (<div className="grid gap-4 md:grid-cols-2">
                ...
                return (<article key={hotel.id} className="flex h-full w-full flex-col rounded-[1.35rem] border border-slate-200 bg-white p-2.5 shadow-[0_16px_38px_-30px_rgba(15,23,42,0.32)] dark:border-slate-800 dark:bg-slate-950 sm:p-3">
                          <div className="flex flex-col gap-3">
                            <div className="relative overflow-hidden rounded-[1rem]">
                              <button type="button" onClick={() => setExpandedHotelGalleryIds((previous) => ({
                        ...previous,
                        [hotelKey]: !previous[hotelKey],
                    }))} className="block w-full text-left" aria-label={isGalleryExpanded ? `Collapse ${hotel.name} photos` : `Expand ${hotel.name} photos`}>
                                <img src={displayImages[0] || hotelImage} alt={hotel.name} className="h-40 w-full object-cover sm:h-48" referrerPolicy="no-referrer"/>
                              </button>
                              {displayImages.length > 1 && (<button type="button" onClick={() => setExpandedHotelGalleryIds((previous) => ({
                            ...previous,
                            [hotelKey]: !previous[hotelKey],
                              ...
                              {isGalleryExpanded && extraImages.length > 0 && (<div className="grid grid-cols-3 gap-1 border-t border-white/10 bg-white p-1 sm:p-1.5 dark:bg-slate-950">
                                  {extraImages.map((image, imageIndex) => (<button key={`${hotel.id}-extra-${imageIndex}-${image}`} type="button" onClick={() => setExpandedHotelGalleryIds((previous) => ({
                                ...previous,
                                [hotelKey]: false,
                            }))} className="overflow-hidden rounded-lg" aria-label={`Collapse gallery for ${hotel.name}`}>
                                      <img src={image} alt={`${hotel.name} photo ${imageIndex + 2}`} className="h-20 w-full object-cover sm:h-24" referrerPolicy="no-referrer"/>
                                    </button>))}
                                </div>)}
                              <button type="button" aria-label={saved ? `Remove ${hotel.name} from saved stays` : `Save ${hotel.name}`} onClick={() => setSavedHotelIds((previous) => ({
                        ...previous,
                        [hotelKey]: !previous[hotelKey],
                              ...
                              <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                                <div className="space-y-2.5">
                                  <div className="flex flex-wrap items-center gap-1.5 text-amber-400">
                                    {[...Array(Math.max(3, Math.min(5, Math.round(ratingValue) || 4)))].map((_, starIndex) => (<Star key={`${hotel.id}-star-${starIndex}`} className="h-3.5 w-3.5 fill-current"/>))}
                                  ...
                                  {hasDiscount && hotel.discount_percentage ? (<div className="inline-flex items-center gap-1.5 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-[11px] font-semibold text-emerald-600 dark:border-emerald-500/30 dark:bg-emerald-500/10">
                        ...
                        return (<span key={`${hotel.id}-${amenity}`} className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-medium text-slate-600 dark:bg-slate-900 dark:text-slate-300">
                                ...
                                <div className="rounded-[1rem] bg-slate-50 px-3 py-2.5 text-right dark:bg-slate-900/80 xl:min-w-[130px]">
                                  ...
                                  {hasDiscount && originalNightlyPrice > nightlyPrice && (<p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-400 line-through">
                ...
                {!hotelsLoading && !hotelsError && sortedHotels.length === 0 && (<div className="rounded-[1.6rem] border border-slate-200 bg-white px-5 py-10 text-center shadow-sm dark:border-slate-800 dark:bg-slate-950">
        ...
        {showMap && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowMap(false)} role="dialog" aria-modal="true" aria-label="Hotel location map">
            <div className="relative h-[80vh] w-[90vw] rounded-xl border border-slate-200 bg-white shadow-lg dark:border-slate-800 dark:bg-slate-950" onClick={(event) => event.stopPropagation()}>


              <button type="button" onClick={() => setShowMap(false)} className="absolute right-3 top-3 rounded-full bg-slate-200 p-1 text-slate-500 hover:bg-slate-300 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600">
                <X className="h-4 w-4"/>
              </button>
              
              <iframe title={`Map for ${selectedHotelForMap?.name || 'selected hotel'}`} src={getOpenStreetMapEmbedUrlForHotel(selectedHotelForMap)} className="h-full w-full rounded-xl border-0" loading="lazy" referrerPolicy="no-referrer-when-downgrade"/>
              {!selectedHotelForMap && (<div className="absolute inset-0 flex items-center justify-center bg-white/70 text-slate-600 dark:bg-slate-900/70 dark:text-slate-300">
