...
import { Shield, Lock, ChevronRight, Calendar, Users, ArrowRight, CheckCircle2, ShieldCheck } from 'lucide-react';
import { AVAILABLE_ACTIVITIES } from '../../data/activities';
const ProcessingStep = ({ label, delay }) => {
...
export const Payment = ({ tripData, onBackToHome, selectedActivityIds = [] }) => {
    // Keep legacy payment-method UI available in-code, but hidden by default.
    const SHOW_LEGACY_PAYMENT_METHODS = false;
    const [selectedMethod, setSelectedMethod] = useState('aba');
    const [paymentStatus, setPaymentStatus] = useState('idle');
    const [showReceipt, setShowReceipt] = useState(false);
    const [transactionAt, setTransactionAt] = useState(null);
    ...
    const stayDatesLabel = checkIn && checkOut ? formatStayRange(checkIn, checkOut) : String(tripData?.dates || 'Check-in / Check-out not set');
    const bookingData = {
    ...
    const LegacyPaymentMethods = () => {
        if (!SHOW_LEGACY_PAYMENT_METHODS)
            return null;
    ...
    if (paymentStatus === 'processing') {
        ...
        <div className="w-full max-w-md text-center">
          ...
          <div className="space-y-4 text-left bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
            ...
            ].map((step, i) => (<ProcessingStep key={i} label={step.label} delay={step.delay}/>))}
          </div>
        </div>
      </div>);
    }
    if (paymentStatus === 'success') {
        return (<div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-4 py-20">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-2xl">
          {/* Success Message */}
          {!showReceipt ? (<div className="text-center">
              ...
              <div className="p-8 md:p-12 space-y-8">
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4 py-6 border-b border-slate-50 dark:border-slate-800">
                    ...
                    {tripData?.rental?.isBooked && (<div className="flex justify-between items-center text-xs">
                        <div className="flex flex-col">
                    ...
                    {selectedActivities.length > 0 && (<div className="space-y-3 pt-2">
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Selected Activities</p>
                        {selectedActivities.map(activity => (<div key={activity.id} className="flex justify-between items-center text-xs">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className={`${SHOW_LEGACY_PAYMENT_METHODS ? 'lg:col-span-4' : 'lg:col-span-12'} space-y-6`}>
            ...
            return (<div className={`grid ${imagesToShow.length === 1 ? 'grid-cols-1' : 'grid-cols-2'} gap-2 h-64 p-2`}>
                      {imagesToShow.map((src, idx) => (<div key={`${src}-${idx}`} className="relative rounded-2xl overflow-hidden bg-slate-100 dark:bg-slate-800">
                          <img src={src} alt={bookingData.title} className="w-full h-full object-cover" referrerPolicy="no-referrer" loading="lazy"/>
                          {extraCount > 0 && idx === imagesToShow.length - 1 && (<div className="absolute inset-0 bg-slate-950/45 flex items-center justify-center">
              ...
              <div className="p-8">
                ...
                <div className="space-y-3 mb-8">
                  ...
                  <div className="pt-4 mt-4 border-t border-slate-50 dark:border-slate-800 space-y-4">
                    ...
                    {/* Activities */}
                    {selectedActivities.length > 0 && (<div>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Activities</p>
                        <div className="space-y-2">
                          {selectedActivities.map(activity => (<div key={activity.id} className="flex justify-between text-[11px]">
