...
export const BookTrip = () => {
    ...
    const [transportId, setTransportId] = React.useState(null);
    const [transports, setTransports] = React.useState([]);
    const [transportError, setTransportError] = React.useState(null);
    ...
    const toDateStart = React.useCallback((value) => {
        if (!value)
            return null;
    ...
    const toDateEnd = React.useCallback((value) => {
        if (!value)
            return null;
        const raw = String(value);
    ...
    const isPromotionActive = React.useCallback((promo) => {
    ...
    const getActivePromotionForTransport = React.useCallback((transportId) => {
        return promotions.find((promo) => {
            if (!isPromotionActive(promo))
                return false;
    ...
    const calculateDiscountedPrice = React.useCallback((basePrice, promotion) => {
    ...
    React.useEffect(() => {
        if (transports.length === 0)
            return;
        if (transportId && transports.some((transport) => transport.id === transportId))
            return;
        setTransportId(transports[0].id);
    }, [transports, transportId]);
    const selectedDestination = React.useMemo(() => destinations.find((destination) => destination.id === destinationId) ?? destinations[0], [destinationId, destinations]);
    const selectedTransport = React.useMemo(() => transports.find((transport) => transport.id === transportId) ?? transports[0], [transportId, transports]);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            try {
                const createdBooking = response?.data ?? payload;
    ...
    return (<div className="px-4 sm:px-6 lg:px-8 pt-28 pb-20 max-w-4xl mx-auto">
      <div className="card p-8">
        ...
        {error && (<div className="mt-6 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl" role="alert">
            {error}
          </div>)}
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          <div>
            ...
            <select className="select-base w-full mt-2" value={destinationId ?? ''} onChange={(event) => setDestinationId(parsePositiveInteger(event.target.value))} disabled={loadingDestinations || destinations.length === 0}>
            ...
            return (<option key={hotel.id} value={hotel.id}>
            ...
            {destinationError && (<p className="mt-2 text-xs text-red-600">{destinationError}</p>)}
          </div>

          <div>
            <label className="block text-sm font-bold text-slate-700 dark:text-slate-200">Transport</label>
            <select className="select-base w-full mt-2" value={transportId ?? ''} onChange={(event) => setTransportId(parsePositiveInteger(event.target.value))} disabled={loadingTransports || transports.length === 0}>
            ...
            {transportError && (<p className="mt-2 text-xs text-red-600">{transportError}</p>)}
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-bold text-slate-700 dark:text-slate-200">Travel date</label>
              <input type="date" className="input-base mt-2" value={travelDate} min={todayDate} onChange={(event) => setTravelDate(event.target.value)}/>
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-700 dark:text-slate-200">Travelers</label>
              <input type="number" min={1} className="input-base mt-2" value={travelers} onChange={(event) => {
          ...
          {/* Price Summary */}
          {(selectedDestination || selectedTransport) && (<div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-200 dark:border-slate-700">
              <h3 className="font-bold text-slate-900 dark:text-white mb-3">Price Summary</h3>
              
              {selectedDestination && (<div className="mb-3">
                  <div className="flex justify-between items-center">
                    ...
                    return (<>
                            {hasDiscount && (<span className="text-xs text-slate-400 line-through block">
                                ${selectedDestination.price.toFixed(2)}
                              </span>)}
                            <span className="font-bold text-slate-900 dark:text-white">
                              ${discountedPrice.toFixed(2)}
                            </span>
                            {promotion && (<span className="text-xs text-emerald-600 ml-2">
              ...
              {selectedTransport && !selectedTransport.is_free && selectedTransport.price_per_km && (<div className="mb-3">
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-slate-600 dark:text-slate-400">
              ...
              {selectedTransport?.is_free && (<div className="mb-3">
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-slate-600 dark:text-slate-400">
