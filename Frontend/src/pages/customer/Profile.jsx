...
import { User, Settings, Bell, Shield, CreditCard, LogOut, Camera, Mail, Phone, MapPin, Globe, Lock, FileText } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { useTheme } from '../../context/ThemeContext';
import { uploadImage } from '../../services/imageService';
export const Profile = ({ initialTab = 'profile', notifications, onMarkAsRead, onMarkAllAsRead, onNotificationClick }) => {
      ...
      <div className="flex flex-col md:flex-row items-center gap-8 bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm">
        ...
        <div className="text-center md:text-left flex-1">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-center md:justify-start gap-3">
            ...
            <button type="button" onClick={() => setIsEditing(true)} className="inline-flex items-center gap-2 rounded-2xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100">
              Edit Profile
            </button>
          </div>
          {profileMessage && (<p className={`mt-3 text-sm font-medium ${profileMessage.type === 'success' ? 'text-emerald-600' : 'text-rose-600'}`}>
              {profileMessage.text}
            </p>)}
        </div>
        <button onClick={() => isEditing ? handleSaveProfile() : setIsEditing(true)} className={`px-6 py-3 rounded-2xl text-sm font-bold transition-all ${isEditing
        ...
        <div className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-slate-700/50">
              ...
              {isEditing ? (<select value={profileData.currency} onChange={(e) => setProfileData({ ...profileData, currency: e.target.value })} className="bg-transparent text-xs font-bold text-emerald-600 outline-none cursor-pointer">
                  <option value="USD ($)">USD ($)</option>
                  <option value="KHR (៛)">KHR (៛)</option>
                </select>) : (<span className="text-xs font-bold text-emerald-600">{profileData.currency}</span>)}
            </div>
          </div>
        </div>
      </div>
    ...
    const renderSettings = () => (<div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm">
        ...
        <div className="flex items-center justify-between p-4 rounded-2xl bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/20">
          ...
          {showDeleteConfirm ? (<div className="flex gap-2">
              <button onClick={handleDeleteAccount} className="px-4 py-2 bg-red-600 text-white text-xs font-bold rounded-xl hover:bg-red-700 transition-colors">
                Confirm
              </button>
              <button onClick={() => setShowDeleteConfirm(false)} className="px-4 py-2 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold rounded-xl hover:bg-slate-300 transition-colors">
                Cancel
              </button>
            </div>) : (<button onClick={() => setShowDeleteConfirm(true)} className="px-4 py-2 bg-red-600 text-white text-xs font-bold rounded-xl hover:bg-red-700 transition-colors">
              Delete
            </button>)}
        </div>
      </div>
    </div>);
    const renderSecurity = () => (<div className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm">
      ...
      <div className="space-y-6">
        <div className="p-6 rounded-2xl bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-700">
          ...
          <button onClick={handleChangePassword} className="w-full py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 text-sm font-bold rounded-xl hover:bg-slate-50 transition-colors">
        ...
        <div className="p-6 rounded-2xl bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-700">
          ...
          <button onClick={handleToggleTFA} className={`w-full py-3 text-sm font-bold rounded-xl transition-colors ${securityStatus.tfaEnabled
    ...
    const renderDocuments = () => (<div className="space-y-8">
      <div className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm">
        <div className="mb-8">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-80 flex-shrink-0">
            <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm p-4 sticky top-28">
              ...
              <button onClick={logout} className="w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-sm font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all">
