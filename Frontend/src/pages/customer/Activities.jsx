...
import { Star, MapPin, Calendar, Users, Check, Waves, Fish, Utensils, Tent, Compass, Camera, Info, ArrowLeft, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const Activities = ({ onBack, onRequireLogin, onConfirmBooking, isAuthenticated, selectedActivityIds, setSelectedActivityIds, }) => {
    ...
    return (<div className="pt-24 pb-20 px-4 sm:px-6 lg:px-8 max-w-[88rem] mx-auto">
      {/* Hero Section */}
      <div className="relative h-[520px] md:h-[600px] rounded-[3rem] overflow-hidden mb-14 shadow-2xl">
        <img src="https://images.unsplash.com/photo-1533587851505-d119e13fa0d7?auto=format&fit=crop&q=80&w=2000" alt="Adventure Activities Banner" className="w-full h-full object-cover" referrerPolicy="no-referrer"/>
        ...
        {onBack && (<button onClick={onBack} className="absolute top-7 left-7 w-12 h-12 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-white/40 transition-all">
      ...
      <div className="grid lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2">
          <section id="activity-options" className="mb-12">
            ...
            <motion.div key={`activity-page-${currentPage}`} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="grid sm:grid-cols-2 gap-4">
              {paginatedOptions.map((option) => (<button key={option.id} onClick={() => toggleOption(option.id)} className={`flex items-start gap-4 p-5 rounded-3xl border-2 transition-all text-left group ${effectiveSelectedIds.includes(option.id)
                ? 'border-blue-600 bg-blue-50/50 dark:bg-blue-900/20'
                : 'border-slate-100 dark:border-slate-800 hover:border-blue-200 dark:hover:border-blue-800'}`}>
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 transition-colors ${effectiveSelectedIds.includes(option.id)
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {effectiveSelectedIds.includes(option.id) && (<div className="w-5 h-5 bg-blue-600 rounded-full flex items-center justify-center">
            ...
            {totalPages > 1 && (<div className="flex items-center justify-center gap-4 mt-8">
                <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="p-3 rounded-2xl border border-slate-100 dark:border-slate-800 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                  <ChevronLeft className="w-5 h-5"/>
                </button>

                <div className="flex items-center gap-2">
                  {Array.from({ length: totalPages }, (_, index) => index + 1).map((page) => (<button key={page} onClick={() => handlePageChange(page)} className={`w-12 h-12 rounded-2xl text-sm font-bold transition-all ${page === currentPage
                ...
                <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="p-3 rounded-2xl border border-slate-100 dark:border-slate-800 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
            ...
            <div className="rounded-[2.75rem] p-[1px] bg-gradient-to-br from-slate-800/70 via-slate-900 to-slate-950 shadow-2xl">
              <div className="bg-gradient-to-b from-slate-900 to-slate-950 rounded-[2.7rem] p-8 border border-white/5">
                ...
                <div className="space-y-4 mb-6">
                  ...
                  {selectedExtras.length > 0 && (<div className="pt-4 border-t border-white/10">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Selected Extras</p>
                      {selectedExtras.map(extra => (<div key={extra.id} className="flex justify-between text-sm mb-2">
                ...
                <button onClick={handleConfirm} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-2xl font-bold shadow-[0_16px_40px_rgba(37,99,235,0.35)] transition-all flex items-center justify-center gap-2 mb-3 active:scale-[0.99]">
                ...
                {showGuestPrompt && (<div className="rounded-2xl border border-amber-400/30 bg-amber-400/10 px-4 py-3 mb-4">
                ...
                {showConfirmed && (<div className="rounded-2xl border border-emerald-400/30 bg-emerald-400/10 px-4 py-3 mb-4">
