...
import { Dashboard as AdminDashboard, UserManagement, OwnerManagement, OwnerDetailsPage, ViewAllApplications, OwnerApplicationDetails, Destinations as AdminDestinations, Bookings as AdminBookings, Finances, AuditLogs, Settings, AdminProfile, } from '@/pages/admin';
const normalizeAdminView = (view) => {
    switch (view) {
        case 'users':
        case 'owners':
...
const OwnerShellInner = ({ onLogout }) => {
    const { user } = useAuth();
    const { isDarkMode, toggleDarkMode } = useTheme();
    const location = useLocation();
    const navigate = useNavigate();
    const [ownerNotifications, setOwnerNotifications] = React.useState([]);
    ...
    const refreshOwnerNotifications = React.useCallback(async () => {
        try {
            ...
            const items = (res?.data ?? res?.data?.data ?? res?.data?.notifications ?? res?.data);
    ...
    const markOwnerNotificationRead = async (id) => {
        try {
    ...
    const openOwnerMessageThread = (notification) => {
        const thread = {
            ...
            ownerName: notification?.data?.conversationName ??
    ...
    const renderOwnerPage = () => {
        ...
        if (path === '/' || path === '/owner')
            return (<OwnerDashboard notifications={ownerNotifications} onOpenBooking={openBookingDetails} onOpenMessageThread={openOwnerMessageThread} onMarkNotificationRead={markOwnerNotificationRead} onViewAllActivities={viewAllActivities}/>);
        if (path.startsWith('/destinations') && path.includes('/add-room'))
            return <OwnerAddRoom />;
        if (path.startsWith('/destinations') && path.includes('/edit'))
            return <OwnerEditProperty />;
        if (path.startsWith('/destinations') && path.includes('/new'))
            return <OwnerAddProperty />;
        if (path.startsWith('/destinations') && path !== '/destinations')
            return <OwnerPropertyDetail />;
        if (path.startsWith('/destinations'))
            return <OwnerDestinations />;
        if (path.startsWith('/transport/new'))
            return <OwnerRegisterVehicle />;
        ...
        if (path.startsWith('/promotions/new'))
            return <OwnerCreatePromotion />;
    ...
    return (<div className="flex h-screen overflow-hidden font-sans transition-colors duration-200">
      <OwnerSidebar />
      <main className="flex-1 flex flex-col overflow-hidden lg:pl-64">
        <Header title={title} isDark={isDarkMode} toggleTheme={toggleDarkMode} notifications={ownerNotifications} onMarkNotificationRead={markOwnerNotificationRead} onMarkAllNotificationsRead={markAllOwnerNotificationsRead} onNotificationClick={(n) => {
            ...
            navigate('/bookings');
        }} onProfileClick={() => { }} onLogoutClick={onLogout} user={user}/>

        <div className="flex-1 overflow-y-auto">
          <AnimatePresence mode="wait">
            <motion.div key={location.pathname} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.22, ease: 'easeOut' }} className="h-full">
              {renderOwnerPage()}
            </motion.div>
          </AnimatePresence>
        </div>
...
const OwnerShell = ({ onLogout }) => {
    return (<OwnerNotificationsProvider>
      <OwnerShellInner onLogout={onLogout}/>
    </OwnerNotificationsProvider>);
};
const getAdminTitle = (view) => {
    switch (view) {
        case 'dashboard':
...
const AdminShell = ({ view, setView, onLogout, }) => {
    const { user } = useAuth();
    const { isDarkMode, toggleDarkMode } = useTheme();
    ...
    const [pendingNavigationView, setPendingNavigationView] = React.useState(null);
    // Don't render if user is not authenticated or not admin
    if (!user || user.role !== 'admin') {
        return null;
    }
    const safeSetView = React.useCallback((nextView) => {
    ...
    const renderAdminPage = () => {
        switch (normalizedView) {
            case 'dashboard':
                return (<AdminDashboard onOpenTotalUsersDetails={() => safeSetView('users')} onOpenTotalOwnersDetails={() => safeSetView('owners')} onOpenTotalBookingsDetails={() => safeSetView('bookings')} onOpenSystemIncomeDetails={() => safeSetView('finances')} onOpenOwnerApplicationsDetails={() => safeSetView('owners-applications')}/>);
            case 'users':
                return <UserManagement />;
            case 'owners':
                return (<OwnerManagement onViewAllApplications={() => safeSetView('owners-applications')} onViewOwnerDetails={() => safeSetView('owners-details')}/>);
            case 'owners-applications':
                return (<ViewAllApplications onBack={() => safeSetView('owners')} onViewDetails={() => safeSetView('owners-applications-details')}/>);
            case 'owners-applications-details':
    ...
    return (<div className="flex h-screen overflow-hidden font-sans transition-colors duration-200">
      <Sidebar activeTab={activeTab} setActiveTab={(tab) => safeSetView(tab)} onProfileClick={() => safeSetView('profile')} onLogout={onLogout} user={user}/>

      <main className="flex-1 flex flex-col overflow-hidden">
        <Header title={getAdminTitle(normalizedView)} isDark={isDarkMode} toggleTheme={toggleDarkMode} onNotificationClick={handleNotificationClick} onProfileClick={() => safeSetView('profile')} onSettingsClick={() => safeSetView('settings')} onLogoutClick={onLogout} user={user}/>
        <div className="flex-1 overflow-y-auto">
          <AnimatePresence mode="wait">
            <motion.div key={normalizedView} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.22, ease: 'easeOut' }} className="h-full">
              {renderAdminPage()}
            </motion.div>
          </AnimatePresence>
        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-md card p-6">
              ...
              <div className="mt-5 flex justify-end gap-3">
                <button onClick={cancelPendingNavigation} className="h-10 px-4 rounded-lg border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-semibold hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  Cancel
                </button>
                <button onClick={confirmPendingNavigation} className="h-10 px-4 rounded-lg bg-amber-500 text-white font-semibold hover:bg-amber-600 transition-colors">
                  Okay
                </button>
              </div>
            </div>
...
export const AppRoutes = ({ view, setView, onSelectRecommendation, onSelectDestination, onSearchDestination, onPromotionsClick, onHotelsClick, onRentalsClick, onActivitiesClick, notifications, onMarkAsRead, onMarkAllAsRead, activeProfileTab, selectedHotel, browseDestination, hotelBackView, setSelectedHotel, selectedActivityIds, setSelectedActivityIds, tripData, setTripData, }) => {
    ...
    const handleReserveHotel = (selection) => {
        if (!requireAuth())
            return;
        setTripData((prev) => {
            const next = { ...(prev || {}) };
            ...
            const destinationId = selectedHotel?.destination_id ??
    ...
    const handleSelectVehicle = (vehicle) => {
        ...
        setTripData((prev) => {
            const next = { ...(prev || {}) };
            const basePrice = typeof vehicle?.price === 'number'
                ? vehicle.price
                : typeof vehicle?.price_per_km === 'number'
                    ? vehicle.price_per_km
                    : Number(vehicle?.price ?? 0);
            const discountedPrice = typeof vehicle?.discounted_price === 'number'
    ...
    const customerBookingRoute = location.pathname === '/customer/book' || location.pathname === '/customer/bookings';
    const publicMapRoute = location.pathname === '/map';
    if (customerBookingRoute) {
        if (isGuest) {
            return (<Login onSwitchToRegister={() => setView('register')} onBack={() => setView('landing')} onSuccess={(nextView) => setView(nextView)}/>);
        }
    ...
    switch (view) {
        case 'login':
            return (<Login onSwitchToRegister={() => setView('register')} onBack={() => setView('landing')} onSuccess={(nextView) => setView(nextView)}/>);
        case 'register':
            return (<Register onSwitchToLogin={() => setView('login')} onBack={() => setView('landing')} onSuccess={(nextView) => setView(nextView)}/>);
        case 'customer-book':
            if (isGuest) {
                return (<Login onSwitchToRegister={() => setView('register')} onBack={() => setView('landing')} onSuccess={(nextView) => setView(nextView)}/>);
            }
        ...
        case 'profile':
              ...
              <div className="relative overflow-hidden rounded-[3rem] border border-slate-200/60 dark:border-white/10 bg-white dark:bg-slate-950 shadow-2xl">
                ...
                <div className="relative p-8 sm:p-12">
                  ...
                  <div className="mt-8 flex flex-col sm:flex-row gap-3">
                    ...
                    <button onClick={() => setView('landing')} className="px-6 py-4 rounded-2xl bg-slate-100 hover:bg-slate-200 dark:bg-white/10 dark:hover:bg-white/15 text-slate-800 dark:text-slate-200 font-bold transition-all">
                      Back to Home
                    </button>
                  </div>
                </div>
            ...
            return (<Profile initialTab={activeProfileTab} notifications={notifications} onMarkAsRead={onMarkAsRead} onMarkAllAsRead={onMarkAllAsRead} onNotificationClick={(notification) => {
                    ...
                    if (String(notification?.type) === 'message' && conversationId) {
                        sessionStorage.setItem('pending_message_thread', JSON.stringify({
                            ownerId: conversationId,
                            ownerEmail: notification?.conversationEmail ?? notification?.data?.conversationEmail ?? '',
                            ownerName: notification?.conversationName ??
        ...
        case 'hotels':
            return (<Hotels tripData={tripData} browseDestination={browseDestination} onBack={() => setView(hotelBackView || 'landing')} onSelectHotel={handleSelectHotel} setTripData={setTripData} onOpenMessages={() => setView('messages')}/>);
        case 'hotel-details':
            return (<HotelDetails tripData={tripData} hotel={selectedHotel} onBack={() => setView('hotels')} onReserve={handleReserveHotel}/>);
        case 'rentals':
            return <Rentals onBack={() => setView('trip-planner')} onSelectVehicle={handleSelectVehicle}/>;
        case 'activities':
            return (<Activities onBack={() => setView('trip-planner')} onRequireLogin={() => setView('login')} onConfirmBooking={() => setView('bookings')} isAuthenticated={!isGuest} selectedActivityIds={selectedActivityIds} setSelectedActivityIds={setSelectedActivityIds}/>);
        case 'promotions':
            return (<Promotions onBack={() => setView('landing')} onClaim={(promotion) => {
                    setTripData((prev) => ({
                        ...prev,
                        promotion
                    }));
        ...
        case 'trip-planner':
            return (<TripPlanner tripData={tripData} setTripData={setTripData} selectedActivityIds={selectedActivityIds} setSelectedActivityIds={setSelectedActivityIds} onBack={() => setView('landing')} onHotelClick={onHotelsClick} onExploreHotel={onHotelsClick} onRentalClick={onRentalsClick} onActivitiesClick={onActivitiesClick} onProceedToBooking={() => {
                    if (!requireAuth())
                        return;
                    setView('bookings');
                }}/>);
        case 'group-planning':
            ...
            return (<GroupPlanning onBack={() => setView('bookings')} tripTitle={tripData?.title}/>);
        case 'bookings':
            if (isGuest) {
                return (<div className="min-h-[70vh] px-4 sm:px-6 lg:px-8 pt-28 pb-20">
            <div className="max-w-5xl mx-auto">
                    ...
                    <div className="w-full lg:w-[360px]">
                      <div className="rounded-[2.5rem] p-[1px] bg-gradient-to-br from-blue-600/40 via-slate-900/40 to-emerald-500/30">
                        <div className="rounded-[2.45rem] bg-slate-900 dark:bg-slate-950 p-7 border border-white/10">
                          ...
                          <div className="mt-6">
                            <button onClick={() => setView('hotels')} className="w-full px-5 py-3 rounded-2xl bg-white/10 hover:bg-white/15 text-slate-100 font-bold transition-all">
                              Browse Hotels
                            </button>
                          </div>
                        </div>
            ...
            return (<BookingHistory onPaymentClick={() => {
                    if (!requireAuth())
                        return;
                    setView('payment');
                }} onHotelClick={onHotelsClick} onRentalClick={onRentalsClick} onGroupPlanningClick={() => {
                    if (!requireAuth())
                        return;
                    setView('group-planning');
                }} selectedActivityIds={selectedActivityIds} setSelectedActivityIds={setSelectedActivityIds} tripData={tripData} setTripData={setTripData}/>);
        case 'group-invite':
            return <GroupInvite />;
        case 'payment':
            if (isGuest)
                return <VisitorHome />;
            return (<Payment tripData={tripData} selectedActivityIds={selectedActivityIds} onBackToHome={() => setView('landing')} onOpenMessages={() => setView('messages')}/>);
        case 'messages':
