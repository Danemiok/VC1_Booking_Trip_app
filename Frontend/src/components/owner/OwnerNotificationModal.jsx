...
export const OwnerNotificationModal = ({ notification, onClose, onOpenBooking }) => {
        ...
        <div className="max-w-3xl mx-auto rounded-[1.75rem] overflow-hidden border border-white/10 bg-slate-900 shadow-2xl max-h-[84vh] flex flex-col">
          <div className="flex items-start justify-between p-5 border-b border-white/10">
            <div className="min-w-0">
              ...
              {createdAtLabel && (<p className="mt-2 text-[11px] font-bold text-slate-400 uppercase">{createdAtLabel}</p>)}
            </div>
            <button onClick={onClose} className="p-2 rounded-xl hover:bg-white/10 text-slate-200 transition-colors" aria-label="Close notification" type="button">
              <X size={18}/>
            </button>
          </div>

          {booking ? (<div className="p-5 space-y-4 overflow-y-auto">
              <div className="flex items-start justify-between gap-4">
                <div>
                  {bookingId && (<p className="text-sm font-extrabold text-white tracking-tight">
                      Booking <span className="text-slate-300">{bookingId}</span>
                    </p>)}
                  {status && (<span className="inline-flex items-center mt-2 px-2.5 py-1 rounded-full bg-amber-500/15 text-amber-300 text-[11px] font-bold">
                      {status}
                    </span>)}
                </div>
              ...
              {image ? (<div className="rounded-2xl overflow-hidden border border-white/10 bg-black/20">
                  <img src={image} alt="Booking" className="w-full h-[180px] object-cover"/>
                </div>) : (<div className="rounded-2xl border border-white/10 bg-white/5 h-[120px] flex items-center justify-center text-slate-400 text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  ...
                  {(booking?.dateStart || booking?.dateEnd) && (<p className="text-xs text-slate-300 mt-1">
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  ...
                  {booking?.paymentMethod && (<p className="text-xs text-slate-400 mt-1">Method: {booking.paymentMethod}</p>)}
                </div>
              </div>

              {booking?.specialRequests && (<div className="rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          <div className="p-4 border-t border-white/10 flex items-center justify-end gap-3">
            <button onClick={onClose} className={cn("px-4 py-2 rounded-xl font-bold text-sm", "bg-white/10 hover:bg-white/15 text-white transition-colors")} type="button">
              Close
            </button>
          </div>
        </div>
