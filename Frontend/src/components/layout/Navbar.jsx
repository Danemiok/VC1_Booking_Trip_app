...
import { ChevronDown, Hotel, Ship, Compass, Bell, CheckCircle2, User, Settings, LogOut, Menu, X, Sun, Moon } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { useTheme } from '../../context/ThemeContext';
import { LogoutConfirmModal } from '../common/LogoutConfirmModal';
import { BrandLogo } from '../common/BrandLogo';
export const Navbar = ({ onLoginClick, user, onLogout, onProfileClick, notifications, onMarkAsRead, onMarkAllAsRead, onHotelsClick, onRentalsClick, onHomeClick, onBookingsClick, onTripPlannerClick, onActivitiesClick, currentView }) => {
    ...
    return (<nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white dark:bg-slate-900 shadow-md py-3' : 'bg-white dark:bg-slate-900 py-4'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-10">
          ...
          <div className="relative" onMouseEnter={() => setIsPlanTripOpen(true)} onMouseLeave={() => setIsPlanTripOpen(false)}>
            ...
            <AnimatePresence>
              {isPlanTripOpen && (<motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute top-full left-0 mt-2 w-64 bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl rounded-[2.5rem] shadow-2xl border border-slate-100 dark:border-white/5 py-4 z-50 overflow-hidden">
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          <button onClick={toggleDarkMode} className="p-2.5 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-all" aria-label="Toggle dark mode">
          ...
          {user && (<div className="relative">
              ...
              <AnimatePresence>
                {isNotificationsOpen && (<motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute top-full right-0 mt-4 w-80 bg-white dark:bg-slate-800 rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-700 py-6 z-50 overflow-hidden">
          ...
          {user ? (<div className="relative">
              ...
              <AnimatePresence>
                {isProfileOpen && (<motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute top-full right-0 mt-4 w-56 bg-white dark:bg-slate-800 rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-700 py-4 z-50">
                    <div className="px-6 py-2 mb-2">
                    ...
                    <div className="h-px bg-slate-100 dark:bg-slate-700 my-2 mx-4"/>
                    <button onClick={handleRequestLogout} className="w-full text-left px-6 py-2.5 text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 font-bold flex items-center gap-3 transition-colors">
      ...
      {/* Mobile Menu Overlay */}
      <AnimatePresence>
        {isMobileMenuOpen && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="lg:hidden bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 shadow-xl overflow-hidden">
            <div className="p-6 space-y-4">
              ...
              {user ? (<div className="pt-6 border-t border-slate-100 dark:border-slate-800">
                  <div className="flex items-center gap-4 px-4 py-4 bg-slate-50 dark:bg-slate-800/50 rounded-[2rem] mb-4">
                    <img src={`https://ui-avatars.com/api/?name=${user.name}&background=0D8ABC&color=fff`} alt={user.name} className="w-12 h-12 rounded-full border-2 border-white dark:border-slate-700"/>
                    <div>
