...
export const SupportModal = ({ isOpen, onClose, contentType }) => {
        ...
        switch (contentType) {
            ...
            case 'terms':
                return {
                    ...
                    content: (<div className="space-y-6 max-h-96 overflow-y-auto pr-2">
              <div>
    ...
    return (<AnimatePresence>
      {isOpen && (<div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} transition={{ duration: 0.3, ease: [0.4, 0, 0.2, 1] }} className="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 max-w-4xl w-full max-h-[85vh] overflow-hidden">
            {/* Header */}
            <div className={`relative p-8 border-b border-slate-200 dark:border-slate-800 bg-gradient-to-r ${isDarkMode ? 'from-slate-900 via-sky-800 to-sky-600' : 'from-slate-800 via-sky-700 to-sky-500'}`}>
              <button onClick={onClose} className="absolute top-6 right-6 p-3 hover:bg-white/20 rounded-xl transition-all duration-200 group">
            ...
            <div className="p-8 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
              <button onClick={onClose} className={`w-full bg-gradient-to-r ${isDarkMode ? 'from-sky-600 to-cyan-500 hover:from-sky-500 hover:to-cyan-400' : 'from-sky-500 to-cyan-500 hover:from-sky-600 hover:to-cyan-600'} text-white px-8 py-4 rounded-2xl font-semibold transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-[1.02]`}>
                Close
              </button>
            </div>
          </motion.div>
