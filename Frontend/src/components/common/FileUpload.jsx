...
export const FileUpload = ({ onUpload, maxSize = 5, acceptedTypes = ['image/jpeg', 'image/png', 'application/pdf'], label = 'Upload Documents', description = 'Drag and drop your files here, or click to browse', disabled = false, className = '', }) => {
    ...
    return (<div className={className}>
      ...
      {errorMessage && (<div className="mt-3 flex items-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-xs font-medium text-red-700 dark:border-red-900/30 dark:bg-red-900/20 dark:text-red-300">
          <AlertCircle className="h-4 w-4"/>
          <span>{errorMessage}</span>
        </div>)}

      <AnimatePresence>
        {files.length > 0 && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="mt-6 space-y-3">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider px-2">Uploaded Files</h4>
            <div className="grid grid-cols-1 gap-3">
              {files.map((entry) => (<motion.div key={entry.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="flex items-center gap-4 p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    event.stopPropagation();
                    removeFile(entry.id);
                }} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-slate-400 hover:text-red-500 transition-colors" aria-label="Remove file">
                      <X className="w-4 h-4"/>
                    </button>
                  </div>
                </motion.div>))}
